...
// Integration tier: forward-apply the phase's filtered real-code diff as ONE
// squash commit onto `origin/<base>` and restore the original branch. No
// history rewrite / filter-branch / force-push (D-06).
//
...
//   6. filter to real changes (rename-aware, D-01);
//   7. D-07 fallback: if nothing real, return { built: false } WITHOUT switching.
//   8. else build: switch -c <clean> origin/<base>, checkout the filtered tree
//      with the exclusion pathspec, `rm -r` any deletion / rename old-path
//      outside the exclusion, make the ONE squash commit, and switch back.
//
...
export async function buildCleanBranch({ cwd, gitFn, phaseNum, phaseName, base }) {
  ...
  // reuse fetchGitData's ACM-only changedFiles; R3) and filter to real changes.
  ...
  // (7) D-07 fallback: a planning-/doc-only phase changes no real files.
  if (real.length === 0) {
  ...
  // (8b) stage the non-excluded A/M content in one copy-from-tree. The excluded
  // `.planning/phases/` side falls out of the `:(exclude)` pathspec naturally.
  await gitFn(cwd, ["checkout", headCommit, "--", ".", EXCLUDE_PATHSPEC]);
  ...
  // copy from the head tree alone would leave them stale (R2). A rename whose
  // oldPath is EXCLUDED needs no rm: the `:(exclude)` checkout leaves it out.
  for (const entry of real) {
    if (entry.status === "D") {
      await gitFn(cwd, ["rm", "-r", "--", entry.path]);
    } else if (entry.status === "R") {
      if (!isExcludedPath(entry.oldPath)) {
