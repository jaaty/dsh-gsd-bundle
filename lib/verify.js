// @dsh-gsd/bundle/verify — the Verify phase tool (opengsd /gsd-verify-work and
// the verify_phase_goal step). Spawns a fresh-context gsd-verifier that reads
// the phase goal, CONTEXT decisions, plans, and execution summaries, and writes
// a VERIFICATION.md with the faithful frontmatter + status decision tree
// (passed | gaps_found | human_needed). The tool reads the status back and
// routes: passed -> suggest gsd_ship; gaps_found -> recommend gsd_repair
// (bounded automatic repair rounds) or manual gsd_plan --gaps closure;
// human_needed -> present the human-verification items.
//
// D-02: verify is recommend-only on gaps_found — it names the repair
// orchestrator in its routing text but never runs repair rounds itself and
// never imports the repair module.

import { defineTool } from "@deepseek-ai/dsh-tools";
import { parseFrontmatter, contextBudget, findToolIn } from "./_shared.js";
import { spawnSubagent, planningContext, cwdOf } from "./_runner.js";
import { VERIFIER_PROMPT } from "./_agents.js";
import { commitArtifacts } from "./_git-artifacts.js";
import { buildCapability } from "./_capabilities.js";

const name = "gsd-verify";
const inject = ["gsdState", "tools", "subagents"];

// Auto-on-verify mempalace capture hook (D-07). A PURE, exported,
// directly-testable helper mirroring runLearningsOnShip in lib/ship.js: it
// takes only { cfg, tools, phase, exec } — NO ctx, NO git, NO gsdState. Gated by
// mempalace.enabled + capture_artifacts, it finds the registered
// gsd_mempalace_capture tool and invokes it with { phase, artifact: "SUMMARY" }
// (OQ-3). Wrapped so a fault never blocks the verify step (onError: skip,
// REQ-MP-06). DEGR-05: if the mempalace plugin is retired (tool absent), the
// hook returns a not-registered/skipped line and never throws.
async function runMempalaceCaptureOnVerify({ cfg, tools, phase, exec }) {
  if (cfg?.mempalace?.enabled !== true || cfg?.mempalace?.capture_artifacts !== true) return "mempalace capture: disabled (mempalace.enabled/capture_artifacts) — skipped";
  const tool = findToolIn(tools, "gsd_mempalace_capture");
  if (!tool) return "mempalace capture: gsd_mempalace_capture not registered — skipped";
  try {
    const r = await tool.execute({ phase, artifact: "SUMMARY" }, exec);
    return "mempalace capture: " + String(r);
  } catch (e) {
    return "mempalace capture: capture failed (non-blocking): " + (e && e.message ? e.message : String(e));
  }
}

function apply(ctx) {
  const gsd = () => ctx.get("gsdState");

  // Publish this plugin's step capability (DEGR-01/D-02). Auto-tracked
  // revertible effect (D-09): retiring the verify plugin withdraws gsdVerify.
  ctx.provide("gsdVerify", buildCapability("gsdVerify"));

  ctx.tools.register(defineTool({
    name: "gsd_verify",
    description: "Verify phase (opengsd /gsd-verify-work): spawn a fresh-context verifier that checks the phase goal was ACTUALLY achieved — requirement coverage, decision coverage, goal alignment, must_haves (truths/artifacts/key_links), anti-patterns — and writes VERIFICATION.md. Verification is NOT just testing. Routes on the status: passed -> ready to ship; gaps_found -> recommend gsd_repair (bounded repair rounds) or re-plan manually with gsd_plan --gaps; human_needed -> present the human-verification items. Prerequisite: gsd_execute has produced SUMMARY.md files.",
    parameters: {
      phase: { type: "number", required: true },
      gaps: { type: "boolean", description: "Re-verification after gap-closure: focus previously-failed items." },
    },
    output: { schema: { type: "string" }, render: (_a, v) => [{ type: "text", text: v }] },
    async execute(args, exec) {
      const cwd = cwdOf(exec);
      const s = gsd();
      if (!s) throw new Error("gsd_verify: gsdState service unavailable");
      if (!(await s.isProject(cwd))) throw new Error("gsd_verify: no .planning/ project — run gsd_init first");
      const subagents = ctx.get("subagents");
      if (!subagents) throw new Error("gsd_verify: `subagents` service unavailable");
      const roadmap = await s.readRoadmap(cwd);
      const phase = (roadmap?.phases || []).find((p) => p.n === args.phase);
      if (!phase) throw new Error(`gsd_verify: phase ${args.phase} not in ROADMAP.md`);
      const { dir: phaseDir, base } = await s.phaseDirAndBase(cwd, args.phase);
      const cfg = await s.readConfig(cwd);
      const maxBudget = contextBudget(cfg?.context_window);

      // gather artefacts
      const project = await s.readProject(cwd);
      const reqs = (await s.readRequirements(cwd)).map((r) => `- [${r.complete ? "x" : " "}] ${r.id}: ${r.text}`).join("\n");
      const contextMd = await s.readArtifact(cwd, args.phase, "CONTEXT").catch(() => "");
      const researchMd = await s.readArtifact(cwd, args.phase, "RESEARCH").catch(() => "");
      const priorVer = await s.readArtifact(cwd, args.phase, "VERIFICATION").catch(() => "");
      const plans = await s.listPlans(cwd, args.phase);
      if (!plans.length) return `gsd_verify: no plans for phase ${args.phase}. Run gsd_plan first.`;
      const summariesMissing = plans.filter((p) => !p.has_summary);
      if (summariesMissing.length) return `gsd_verify: missing SUMMARY.md for ${summariesMissing.map((p) => p.id).join(", ")}. Run gsd_execute first.`;

      const planContents = [];
      const summaryContents = [];
      for (const p of plans) {
        planContents.push({ label: `PLAN ${p.id}`, content: await s.readArtifact(cwd, args.phase, `PLAN-${String(p.plan).padStart(2, "0")}`).catch(() => "") });
        summaryContents.push({ label: `SUMMARY ${p.id}`, content: await s.readArtifact(cwd, args.phase, `SUMMARY-${String(p.plan).padStart(2, "0")}`).catch(() => "") });
      }

      const notes = [];
      const pc = planningContext(
        [
          { label: "PROJECT.md", content: project },
          { label: "ROADMAP phase goal + success criteria", content: `Phase ${args.phase}: ${phase.name}\nGoal: ${phase.goal}\nRequirements: ${(phase.requirements || []).join(", ")}` },
          { label: "REQUIREMENTS.md", content: reqs },
          { label: "CONTEXT.md", content: contextMd },
          { label: "RESEARCH.md", content: researchMd },
          ...(priorVer ? [{ label: "prior VERIFICATION.md", content: priorVer }] : []),
          ...planContents,
          ...summaryContents,
        ],
        60000, maxBudget,
      );
      if (pc.truncated.length) notes.push(`planning-context: truncated ${pc.truncated.length} ${pc.truncated.length === 1 ? "entry" : "entries"} (${pc.truncated.map((t) => t.label).join(", ")}) — capping total context to ${maxBudget} chars`);
      const prompt = [
        VERIFIER_PROMPT,
        pc.text,
        args.gaps ? "\nRE-VERIFICATION MODE: focus previously-failed items; quick regression on passed truths." : "",
        `\nWrite VERIFICATION.md with the Write tool to ${phaseDir}/${base}-VERIFICATION.md. DO NOT commit it. Return: status, score, and the report path.`,
      ].filter(Boolean).join("\n\n");

      const r = await spawnSubagent(ctx, exec, { label: `verify phase ${args.phase}`, promptText: prompt });
      // read back the status from the written file (authoritative)
      const verText = await s.readArtifact(cwd, args.phase, "VERIFICATION").catch(() => "");
      let status = "gaps_found";
      let score = "";
      if (verText) {
        const { frontmatter } = parseFrontmatter(verText);
        if (frontmatter.status) status = String(frontmatter.status);
        if (frontmatter.score != null) score = String(frontmatter.score);
      } else {
        // CR-07: the verifier completed but wrote no readable VERIFICATION.md —
        // do NOT degrade to the optimistic gaps_found default (P2/R4; repair.js's
        // readVerificationStatus distinguishes missing-file for the same reason).
        // A missing report is not "gaps found": route as its own outcome so the
        // human (and any autonomous run) re-runs verify instead of chasing
        // phantom gaps that repair's gate would then stop on as missing-file.
        status = "missing-report";
      }
      await s.setActivePhase(cwd, args.phase, status === "passed" ? "ship" : "verify");
      const commit = await commitArtifacts(cwd, args.phase, { scope: "verify", phaseName: phase.name });

      // D-07: mempalace capture at verify:post (best-effort, never blocks the step
      // — runMempalaceCaptureOnVerify catches any tool fault and returns a
      // non-blocking log line, REQ-MP-06). Files SUMMARY.md verbatim (OQ-3).
      const mempalaceLine = await runMempalaceCaptureOnVerify({ cfg, tools: ctx.tools, phase: args.phase, exec });

      const route = {
        passed: `✓ Phase ${args.phase} verified (score ${score || "n/a"}). The phase is closed — replanning is blocked without --force. Next: gsd_ship on phase ${args.phase}.`,
        gaps_found: `✗ Phase ${args.phase}: gaps found (score ${score || "n/a"}). Next: gsd_repair on phase ${args.phase} to run bounded automatic recovery rounds (gsd_plan --gaps → gsd_execute --gaps-only → gsd_verify --gaps), or produce fix plans manually with gsd_plan --gaps.`,
        human_needed: `⚠ Phase ${args.phase}: human verification needed (score ${score || "n/a"}). Review the human_verification items in ${phaseDir}/${base}-VERIFICATION.md and confirm before re-verifying.`,
        "missing-report": `⚠ Phase ${args.phase}: the verifier produced no readable VERIFICATION.md — nothing was verified, so there is nothing to repair. Re-run gsd_verify on phase ${args.phase}.`,
      }[status] || `Phase ${args.phase}: verification status "${status}". See ${phaseDir}/${base}-VERIFICATION.md.`;

      return [
        route,
        "",
        `Verification artefacts committed: ${commit.committed} (${commit.staged.length} file(s)).${commit.warning ? " WARNING: " + commit.warning : ""}`,
        `Verifier output: ${r.output.slice(0, 500)}`,
        r.diagnostic ? `Diagnostic: ${r.diagnostic}` : "",
        mempalaceLine,
        ...notes,
      ].filter(Boolean).join("\n");
    },
    presentCall: (a) => ({ card: "generic", title: `Verify phase ${a.phase}`, kind: "other", rawInput: { phase: a.phase } }),
  }));
}

export { name, inject, apply, runMempalaceCaptureOnVerify };