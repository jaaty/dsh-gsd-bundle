...
// Exported so code-review.js (and other tools) can reuse the same seam for
// per-fix source commits and git-diff scoping.
export async function defaultGitFn(cwd, args) {
...
// Per-fix atomic commit of SPECIFIC SOURCE FILES (D-11 / R-1). Unlike
// commitArtifacts (which stages `.planning` wholesale), this stages the exact
// file paths passed in `files` — source files in lib/, test/, plugins/ that a
// code-review fix touches. Same injectable `gitFn` seam and fixed `-C cwd`
// argument-array security discipline as commitArtifacts. Never throws —
// swallows no-git / nothing-staged / add / commit failures with a warning.
//
// Returns { committed, staged, message, warning? }.
export async function commitSourceFiles(cwd, files, message, gitFn = defaultGitFn) {
  try {
    await gitFn(cwd, ["add", ...files]);
  } catch (e) {
    return { committed: false, staged: [], message, warning: `git add failed: ${e.message}` };
  }
  ...
  if (!staged.length) {
    return { committed: false, staged: [], message, warning: "nothing staged" };
  }
