...
const inject = ["fs", "gsdState", "tools", "subagents"];
...
// The structured-output contract for the gsd-code-fixer subagent (D-02: bounded
// anchor-edit contract). The subagent returns VERBATIM unique { find, replace }
// anchor edits; the TOOL (not the subagent) validates each anchor against the
// real file content, applies the edits in memory, writes once, and commits
// atomically via commitSourceFiles (D-01/D-12: tool-driven, not fixer-driven).
// Restricted object-rooted subset (no pattern/format/numeric bounds). "edits"
// is conditionally required and enforced tool-side (OQ-6) — the restricted
// subset has no if/then — so top-level required stays [id, status, file].
...
// Apply the fixer's ordered anchor edits to in-memory content (D-01/D-02/D-03).
// Pure, fs-free helper (CONTEXT code_context locks this exact signature):
// applyAnchorEdits(content, edits) → { content, failed }. Edits apply IN ORDER,
// each validated against the EVOLVING in-memory result of the previous edits
// (edit N sees edits 1..N-1 applied — OQ-9). Each edit's `find` must occur
// EXACTLY ONCE (non-overlapping split count) in the current in-memory content —
// an empty `find` counts as 0 and is rejected. On any violation the ORIGINAL
// input is returned byte-identical with a truthy `failed` cause naming the edit
// index and the match count (D-03: no partial application ever escapes; the
// caller writes nothing when failed). Exported for direct unit testing.
export function applyAnchorEdits(content, edits) {
  const original = String(content ?? "");
  if (!Array.isArray(edits)) {
    return { content: original, failed: "edits is not an array" };
  }
  let out = original;
  for (let i = 0; i < edits.length; i++) {
    const edit = edits[i];
    if (!edit || typeof edit !== "object") {
      return { content: original, failed: `edit ${i + 1} is not an object` };
    }
    const find = String(edit.find ?? "");
    const count = find.length === 0 ? 0 : out.split(find).length - 1;
    if (count === 0) {
      return { content: original, failed: `edit ${i + 1}: anchor matched 0 times` };
    }
    if (count > 1) {
      return { content: original, failed: `edit ${i + 1}: anchor matched ${count} times (must be exactly once)` };
    }
    // Function replacement so `$`-patterns in `replace` are literal text, and
    // only the (single) matched occurrence is replaced.
    out = out.replace(find, () => String(edit.replace ?? ""));
  }
  return { content: out, failed: null };
}

// Truncate a diagnostic/error string to a bounded excerpt (OQ-12), mirroring
// the local helper at lib/repair.js:74. Used for skip causes so REVIEW-FIX.md
// carries the REAL fault text (raw stopReason + diagnostic excerpt), never a
// generic placeholder. Exported for direct unit testing.
export function excerpt(text, max = 400) {
  const t = String(text ?? "").trim();
  return t.length > max ? `${t.slice(0, max)}…` : t;
}

// The two fixer-infrastructure fault causes, VERBATIM from spawnSubagent
// (lib/_runner.js:10,12). D-06 reserves 'unavailable' for exactly this fault
// class — a reviewer fault or a per-finding fixer fault is never 'unavailable'.
const INFRA_SERVICE_CAUSE =
  "gsd: the `subagents` service is unavailable — the bundle needs the host spawn provider (@deepseek-ai/dsh-subagent + dsh-subagent-spawn-in-process)";
const INFRA_PROVIDER_CAUSE =
  "gsd: the `spawn` subagent provider is not registered — install/enable @deepseek-ai/dsh-subagent-spawn-in-process";

// D-06 decision table as a pure function: the overall REVIEW-FIX.md status.
// 'unavailable' when fixer infrastructure is absent; 'applied' when at least
// 1 fix committed; 'skipped' when the loop ran but nothing landed (every skip
// carries a cause). skippedFixes is part of the locked signature (D-06 table
// context) — the skip causes are rendered by the report writer, not here.
export function resolveFixStatus(appliedFixes, skippedFixes, fixUnavailable) {
  if (fixUnavailable) return "unavailable";
  if ((appliedFixes || []).length >= 1) return "applied";
  return "skipped";
}

// Injectable child-process seam for the parse gate (the runPreflightVerify
// pattern, lib/preflight-verify.js:22): production uses the promisified
// execFile; tests swap in a recording/failing execFileFn. A test seam only —
// not a tool/command/capability/config key, so no surface counts move (D-10).
let parseGateExecFileFn = execFileP;
export function setParseGateExecFileFn(fn) {
  parseGateExecFileFn = fn || execFileP;
}

// D-03 parse gate: verify post-edit .js/.mjs content still parses via
// `node --check` on a temporary .mjs copy. A REAL file is required (a real
// child process checks it) — the sanctioned exception to artefacts-via-ctx.fs,
// mirroring lib/preflight-verify.js: the temp dir is created with mkdtemp under
// os.tmpdir() (prefix gsd-parse-, NEVER inside the workspace or .planning/) and
// removed with rm recursive+force in a finally. The child process runs through
// an argument-array execFile — never shell interpolation (same discipline as
// lib/_git-artifacts.js:14-16). Non-JS targets get anchor validation only
// (checked: false, no child process). Returns { ok, checked, cause? }:
//   - { ok: true, checked: false }            → non-.js/.mjs target, skipped gate
//   - { ok: true, checked: true }             → node --check passed
//   - { ok: false, checked: true, cause }     → parse failure (stderr excerpt)
//   - { ok: false, checked: false, cause }    → temp-dir/write infrastructure fault
export async function parseGate(content, targetFile, execFileFn = execFileP) {
  const file = String(targetFile || "");
  if (!/\.(js|mjs)$/.test(file)) return { ok: true, checked: false };
  let dir = null;
  try {
    dir = await mkdtemp(path.join(os.tmpdir(), "gsd-parse-"));
    const tmp = path.join(dir, "gsd-check.mjs");
    await writeFile(tmp, String(content ?? ""), "utf8");
    try {
      await execFileFn(process.execPath, ["--check", tmp], { encoding: "utf8" });
      return { ok: true, checked: true };
    } catch (e) {
      return { ok: false, checked: true, cause: excerpt(String(e.stderr || e.stdout || e.message || "")) };
    }
  } catch (e) {
    return { ok: false, checked: false, cause: excerpt(String((e && e.message) || e)) };
  } finally {
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // applies fixes per-finding via a gsd-code-fixer subagent returning
      // bounded anchor edits. The TOOL validates the anchors, applies the
      // edits, writes the file once, and commits atomically via
      // commitSourceFiles (D-12: tool-driven, not fixer-driven). Degrades to
      // UNAVAILABLE REVIEW-FIX.md on fixer fault (D-09). --auto re-spawns the
      // reviewer after each fix round, up to MAX_ITERATIONS (3), stopping
      // early when a re-review yields no BLOCKER/WARNING findings (D-06).
      ...
      // Reusable fix loop: spawns a fixer subagent per fixable finding, applies
      // the returned anchor edits tool-side (validate → apply-in-memory → write
      // once), commits atomically with a real rev-parse hash capture, and
      // accumulates results (D-01/D-02/D-03/D-11/D-12).
      async function runFixLoop(currentFindings, includeInfo, appliedAccum, skippedAccum) {
        ...
        // gitFn is not a host-provided service; in an active fiber ctx.gitFn
        // throws "without inject". Guard so the --fix path falls back to the
        // default git seam (mirrors the inactive-fiber behaviour of the other
        // tools) while still honouring a host-provided gitFn when present.
        ...
        // Fixer-infrastructure probe (OQ-3/D-06), BEFORE the findings loop:
        // mirrors exactly the two checks spawnSubagent performs
        // (lib/_runner.js:9-12). When the subagents service or its spawn
        // provider is absent, EVERY fixable finding is recorded 'unavailable'
        // with the verbatim infra cause and nothing spawns. 'unavailable' is
        // reserved for exactly this fault class (D-06); per-finding faults in
        // the loop below are skip-and-continue instead.
        let subSvc = null;
        try { subSvc = ctx.get("subagents"); } catch { subSvc = null; }
        if (!subSvc) {
          fixUnavailable = true;
          fixErrorCause = INFRA_SERVICE_CAUSE;
          for (const f of fixable) {
            skippedAccum.push({ id: f.id, file: f.file, severity: f.severity, status: "unavailable", reason: fixErrorCause });
          }
          return { fixUnavailable, fixErrorCause };
        }
        const spawnProvider = subSvc.getProvider ? subSvc.getProvider("spawn") : undefined;
        if (!spawnProvider) {
          fixUnavailable = true;
          fixErrorCause = INFRA_PROVIDER_CAUSE;
          for (const f of fixable) {
            skippedAccum.push({ id: f.id, file: f.file, severity: f.severity, status: "unavailable", reason: fixErrorCause });
          }
          return { fixUnavailable, fixErrorCause };
        ...
        for (let i = 0; i < fixable.length; i++) {
          ...
          // Defense-in-depth path guard (OQ-8): the fix target is ALWAYS
          // finding.file — a reviewer-sourced path that fails traversal /
          // metachar validation is skipped before any read, spawn, or write.
          const pathCheck = validateFiles([finding.file], cwd);
          if (!pathCheck.valid.length) {
            skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: `invalid fix target path: ${finding.file}` });
            continue;
          }

          let currentContent = "";
          let existedBefore = false; // OQ-5: the file-existence signal the old catch discarded
          try {
            const fileTarget = await ctx.fs.resolve(`${cwd}/${finding.file}`);
            const prior = await ctx.fs.readText(fileTarget);
            if (prior != null) { existedBefore = true; currentContent = prior; }
          ...
          try {
            ...
            // OQ-1: treat only the literal "completed" as success (the repo's
            // own runtime convention, lib/jobs.js:174) — never enumerate host
            // stopReason values. D-06/D-07: this finding is skipped with its
            // REAL cause (raw stopReason + diagnostic excerpt) and the loop
            // continues; exactly ONE fixer attempt per finding, no retry.
            if (fr.stopReason !== "completed") {
              const diag = excerpt(fr.diagnostic || fr.output || "");
              skippedAccum.push({
                id: finding.id, file: finding.file, severity: finding.severity, status: "skipped",
                reason: `fixer did not complete (stopReason: ${fr.stopReason})${diag ? ` — ${diag}` : ""}`,
              });
              continue;
            }
            const fixResult = fr.structured;
            if (!fixResult || typeof fixResult !== "object") {
              // D-06: replace the old generic "malformed output" string with
              // the real cause — the absent structured output + the stopReason.
              const diag = excerpt(fr.diagnostic || fr.output || "");
              skippedAccum.push({
                id: finding.id, file: finding.file, severity: finding.severity, status: "skipped",
                reason: `fixer returned no structured output (stopReason: ${fr.stopReason ?? "unknown"})${diag ? ` — ${diag}` : ""}`,
              });
            ...
            } else if (fixResult.status === "fixed") {
              // Structural validation (OQ-6): status "fixed" requires a non-empty
              // edits array — the restricted schema subset cannot express if/then.
              if (!Array.isArray(fixResult.edits) || fixResult.edits.length === 0) {
                skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: "fixer returned no edits" });
              } else if (fixResult.id != null && fixResult.id !== finding.id) {
                // Integrity check (OQ-7): never attribute output to the wrong finding.
                skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: `fixer returned output for finding ${fixResult.id}` });
              } else if (fixResult.file != null && fixResult.file !== finding.file) {
                // OQ-8: the fix target is ALWAYS finding.file — a mismatched
                // fixer-echoed path is a fault, never a redirect.
                skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: `fixer returned file ${fixResult.file} but the finding targets ${finding.file}` });
              } else {
                // Apply the edits to in-memory content (D-03: abort BEFORE any
                // write — a failed anchor validation leaves the file untouched).
                const result = applyAnchorEdits(currentContent, fixResult.edits);
                if (result.failed) {
                  skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: result.failed });
                } else {
                  // D-03 parse gate (.js/.mjs only; non-JS targets get anchor
                  // validation only): a failing parse aborts BEFORE any write —
                  // the file stays byte-identical and the finding is skipped
                  // with the stderr cause (OQ-10: real file under os.tmpdir(),
                  // argument-array execFile, temp removed in a finally).
                  const gate = await parseGate(result.content, finding.file, parseGateExecFileFn);
                  if (!gate.ok) {
                    skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: `parse check failed: ${gate.cause || "unknown"}` });
                  } else {
                    // Write once (D-03 atomicity). The fixer-echoed file is never
                    // the write target (OQ-8) — finding.file is.
                    const fileTarget = await ctx.fs.resolve(`${cwd}/${finding.file}`);
                    await ctx.fs.writeText(fileTarget, result.content);
                    // Per-fix atomic commit + real commit-hash capture (D-12).
                    const r = await commitSourceFiles(cwd, [finding.file], scopedMessage, gitFn);
                    if (r.committed) {
                      let hash = null;
                      let hashNote = null;
                      try {
                        hash = (await gitFn(cwd, ["rev-parse", "HEAD"])).trim();
                      } catch (e) {
                        // OQ-11: the commit DID land — never downgrade to
                        // skipped. Keep status "fixed" with an unresolved-hash
                        // note so fixes_applied stays truthful.
                        hashNote = `commit hash unresolved: ${((e && e.message) || String(e))}`;
                      }
                      appliedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "fixed", commit: hash, note: hashNote });
                    } else {
                      // commitSourceFiles warning (D-12/OQ-5): the write landed
                      // but no commit did — restore disk state so a "fixed" row
                      // always implies a landed commit, then record the warning
                      // as the skip cause with NO hash.
                      try {
                        if (existedBefore) await ctx.fs.writeText(fileTarget, currentContent);
                        else await ctx.fs.unlink(fileTarget);
                      } catch { /* best-effort restore — the report already carries the cause */ }
                      skippedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "skipped", reason: r.warning || "commit failed" });
                    }
                  }
                }
              }
            } else {
              // Structured object present but an invalid status value — name
              // the actual value, not a generic string (D-06 real cause).
              skippedAccum.push({
                id: finding.id, file: finding.file, severity: finding.severity, status: "skipped",
                reason: `fixer returned malformed structured output (status: ${String(fixResult.status)})`,
              });
            }
          } catch (e) {
            // D-06 skip-and-continue: a fixer fault (spawn error, rejected run,
            // anything thrown) isolates to THIS finding with its real cause and
            // the loop continues to the next finding. NOT 'unavailable' — that
            // stays reserved for fixer-infrastructure absence (probe above).
            skippedAccum.push({
              id: finding.id, file: finding.file, severity: finding.severity, status: "skipped",
      ...
      async function writeReviewFixMd(appliedFixes, skippedFixes, fixUnavailable, fixErrorCause) {
        // D-06 decision table as a pure helper: unavailable > applied ≥1 > skipped.
        fixStatus = resolveFixStatus(appliedFixes, skippedFixes, fixUnavailable);
        // D-11: the list of real per-fix commit hashes harvested from the
        // applied rows that carry one (stringifyFrontmatter renders inline
        // arrays; parseFrontmatter round-trips them).
        const commits = appliedFixes.filter((f) => f.commit).map((f) => f.commit);
        const fixFm = {
          phase: String(args.phase),
          fixed: nowIso(),
        ...
        if (fixStatus === "skipped") {
          fixBodyLines.push("_No fix landed in this run — every skipped fix's real cause is listed below._", "");
        }
        if (appliedFixes.length) {
          fixBodyLines.push("## Applied Fixes", "");
          for (const f of appliedFixes) {
            // OQ-11: a landed commit whose rev-parse failed renders its
            // unresolved-hash note instead of a hash.
            fixBodyLines.push(`- **${f.id}** [${f.severity}] ${f.file} — fixed${f.commit ? ` (commit ${f.commit})` : ""}${f.note ? ` — ${f.note}` : ""}`);
          }
          fixBodyLines.push("");
        }
      ...
      if (flags.fix) {
        ...
        if (!findings || findings.length === 0) {
          ...
          if (flags.auto && !fixUnavailable) {
            ...
            while (iteration < MAX_ITERATIONS) {
              ...
              if (reReview.status === "UNAVAILABLE") {
                // OQ-4: a reviewer fault is NOT a fixer-infrastructure fault —
                // D-06 reserves 'unavailable' for the latter. Stop the
                // iteration loop with the already-accumulated fix results
                // standing; the overall status is computed by resolveFixStatus
                // from fix outcomes. REVIEW.md has already been overwritten
                // with the UNAVAILABLE re-review above — leave that as-is.
                autoNote = ` --auto: stopped after iteration ${iteration} — re-review faulted (${reReview.errorCause}).`;
                break;
              }
            ...
            // A re-review-fault stop already wrote its autoNote — the
            // max-iterations note must not overwrite it (OQ-4).
            if (!converged && !fixUnavailable && !autoNote) {
          ...
          // D-06 wording: a skipped run points at the real causes recorded in
          // REVIEW-FIX.md; the applied/unavailable wording stays intact.
          fixSummary = ` REVIEW-FIX.md written (${appliedFixes.length} applied, ${skippedFixes.length} skipped, status ${fixStatus}).${fixStatus === "skipped" ? ` Every skipped fix's real cause is recorded in REVIEW-FIX.md.` : ""}${autoNote}`;
          await s.addDecision(cwd, `Phase ${args.phase}: REVIEW-FIX.md written (${appliedFixes.length} fixes applied, ${skippedFixes.length} skipped, status ${fixStatus})`);
        }
      }
