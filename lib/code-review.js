              ...
              if (!Array.isArray(fixResult.edits) || fixResult.edits.length === 0) {
                ...
                if (result.failed) {
                  ...
                  if (r.committed) {
                    ...
                    } catch { /* rev-parse fault must not crash the loop */ }
                    appliedAccum.push({ id: finding.id, file: finding.file, severity: finding.severity, status: "fixed", commit: hash });
                  } else {
                    // commitSourceFiles warning (D-12): recorded as the fix's
                    // outcome cause instead of a hash.
      ...
      async function writeReviewFixMd(appliedFixes, skippedFixes, fixUnavailable, fixErrorCause) {
        ...
        if (appliedFixes.length) {
          ...
          for (const f of appliedFixes) {
            fixBodyLines.push(`- **${f.id}** [${f.severity}] ${f.file} — fixed${f.commit ? ` (commit ${f.commit})` : ""}`);
          }
          fixBodyLines.push("");
        }
