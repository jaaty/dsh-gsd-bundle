...
export function classifyNextState(snapshot, descriptors) {
  ...
  if (s.state && s.roadmap) {
    ...
    if (activePhase !== null && activePhase !== undefined && String(activePhase) !== "") {
      ...
      // D-07 fall-through: done status OR Complete phase -> fall through to
      // branch 5 (phase-shipped-next), never a no-op recommendation.
  ...
  if (pending.length > 0) {
    const nextPhaseNum = Math.min(...pending.map((p) => p.n));
