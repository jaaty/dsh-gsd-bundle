...
import { matchesGapClosure, parseFrontmatter, zeroPad, findToolIn } from "./_shared.js";
...
// CR-05: the stop-classification contract between repair and its consumers
// (autonomous.js) is a shared SYMBOL, not a magic English substring restated
// across modules. The budget-exhaustion cause template below embeds this
// phrase, and autonomous matches against this constant — if the wording ever
// changes, both sides change together and an early stop can never be silently
// reclassified as budget exhaustion.
export const BUDGET_EXHAUSTED_PHRASE = "repair budget exhausted";
...
// lives on exactly one source line — derived from REPAIR_ROUND_BUDGET so
// raising the budget cannot leave a stale upper bound behind (CR-08).
export const ROUNDS_DOMAIN_MSG = `rounds must be an integer between 1 and ${REPAIR_ROUND_BUDGET}`;

// Tool lookup: findToolIn from _shared.js (dual-shape: array harness / service
// runtime, CR-03). Previously re-declared locally because quick.js imported
// autonomous.js and a quick.js import would have cycled — _shared.js is a leaf
// module, so the single-sourced helper is cycle-safe and both shapes cannot
// drift again.
...
async function runOneRound({ cwd, s, ctx, exec, phaseNum }) {
  ...
  const planTool = findToolIn(ctx.tools, "gsd_plan");
  ...
  const executeTool = findToolIn(ctx.tools, "gsd_execute");
  ...
  if (unfinished.length) {
    ...
    // agent regex-detects — surface it verbatim, never swallow it (P5). It is
    // extracted BEFORE the checkpoint branch so BOTH stop causes carry it: in
    // the real awaiting flow the persisted CHECKPOINT artefact always exists
    // (lib/_checkpoint.js: awaiting is true only when checkpointFm exists), so
    // a marker extracted only in the marker-only branch below would be dead
    // code there and the decision_id the resumer must echo back would be
    // truncated away by the 400-char excerpt (CR-02).
    ...
    for (const p of unfinished) {
      if (await s.hasArtifact(cwd, phaseNum, `CHECKPOINT-${zeroPad(Number(p.plan))}`)) {
        return {
          ...
          stopCause: `execute stopped at a resumable checkpoint on plan ${p.id} (no SUMMARY written) — resume it through gsd_execute's answer/decision_id channel, then re-invoke gsd_repair.${markerLine ? ` ${markerLine}` : ""} Execute output excerpt: ${excerpt(execOut)}`,
        };
      }
    }
    if (markerLine) {
  ...
  const verifyTool = findToolIn(ctx.tools, "gsd_verify");
...
function roundSection(phaseNum, attempt, round) {
  ...
  const delegated = [];
  if (round.ranPlan) {
    steps.push("plan(gaps)");
    delegated.push(`plan({ phase: ${phaseNum}, gaps: true })`);
  }
  if (round.ranExecute) {
    steps.push("execute(gaps-only)");
    delegated.push(`execute({ phase: ${phaseNum}, gapsOnly: true })`);
  }
  if (round.ranVerify) {
    steps.push("verify(gaps)");
    delegated.push(`verify({ phase: ${phaseNum}, gaps: true })`);
  }
  const lines = [
    `## Round ${attempt}`,
    `- attempt: ${attempt}`,
    // CR-09: the delegated-args clause is built from the round's ran flags, so
    // a round that stopped at plan never logs execute()/verify() signatures it
    // never reached — the record and the delegation claim always agree.
    `- actions: ${steps.join(" → ")} — delegated as ${delegated.join(" → ")}`,
...
export async function runRepairRounds({ cwd, s, ctx, exec, phaseNum, phaseName, rounds, gitFn }) {
  ...
  // (3) The bounded budget loop (D-06/D-07): up to `rounds` attempts of the
  // same strict-order round body (plan → execute → verify), each round only
  // attempted when the previous step of that round succeeded. `rounds`
  // defaults to the hard budget when omitted; the defensive guard above has
  // already pinned any explicit value to 1..REPAIR_ROUND_BUDGET.
  const budget = rounds == null ? REPAIR_ROUND_BUDGET : rounds;
  const sections = [];
  let attempt = 0;
  let recovered = false;
  let stopReason = null;
  let finalStatus = "gaps_found";

  // CR-04: a delegated tool can THROW mid-round (fail-loud by design, R6 —
  // e.g. a corrupt/out-of-range CHECKPOINT artefact makes gsd_execute reject),
  // and runOneRound deliberately propagates that. The attempted round must
  // still leave its audit record: on a throw, record a `## Stop` section with
  // the real cause, land REPAIR.md + the scope-repair commit through the same
  // epilogue, then rethrow so the fail-loud contract (and the autonomous
  // driver's repair-fault branch) still sees the original error.
  try {
    while (attempt < budget) {
      ...
      if (round.stopCause) {
        // D-11: an in-round stop consumes the invocation immediately — no blind
        // retry of the same round; the cause names the real failure only. A
        // mid-task execute failure hands off to the checkpoint/resume path via
        // the surfaced cause rather than re-running the plan from scratch.
        stopReason = round.stopCause;
        finalStatus = round.verifyStatus || "gaps_found"; // unchanged when stopped before verify
        break;
      }
      if (round.verifyStatus === "passed") {
        recovered = true;
        finalStatus = "passed";
        break;
      }
      // gaps_found after this round: start the next round while budget remains;
      // when exhausted, stop and report the remaining gaps instead of looping
      // (D-07). The exhaustion phrase (BUDGET_EXHAUSTED_PHRASE) is the designed
      // stop-classification contract — it lives ONLY here, never in an early
      // stop cause.
      if (attempt < budget) continue;
      finalStatus = "gaps_found";
      stopReason = `${BUDGET_EXHAUSTED_PHRASE} (${attempt} of ${budget} round(s)) — the remaining gaps are listed in the phase's VERIFICATION.md. Verify output excerpt: ${excerpt(round.output)}`;
      break;
    }
  } catch (err) {
    stopReason = `gsd_repair: delegate failed in round ${attempt} — ${err && err.message ? err.message : String(err)}`;
    sections.push(stopSection(stopReason));
    await epilogue({ roundsRun: attempt, finalStatus, sections });
    throw err;
  }
