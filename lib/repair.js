...
export async function runRepairRounds({ cwd, s, ctx, exec, phaseNum, phaseName, rounds, gitFn }) {
  ...
  try {
    while (attempt < budget) {
      ...
      // (D-07). The exhaustion phrase is the designed stop-classification
      // contract — it lives ONLY here, never in an early stop cause.
      if (attempt < budget) continue;
      finalStatus = "gaps_found";
      stopReason = `repair budget exhausted (${attempt} of ${budget} round(s)) — the remaining gaps are listed in the phase's VERIFICATION.md. Verify output excerpt: ${excerpt(round.output)}`;
      break;
    }
  } catch (err) {
