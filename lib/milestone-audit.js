...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // Cursor guard (state-repair 2026-09-05): when the milestone is
      // ready-to-close, every phase is Complete, so no phase should be active.
      // Reconcile the loop cursor to idle to clear any stale active_phase left by
      // a re-verify during the audit pass. This is NOT setActivePhase — it only
      // resets to the idle baseline and never advances to a phase step.
      if (status === "ready-to-close") {
        await s.reconcileToIdle(cwd);
      }
