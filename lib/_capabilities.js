...
// The 22 known capability keys, in a stable order: the onboarding map step
...
// milestone-audit (order 52) slots after ship(50) per phase 43 D-01; learnings
// (order 53) slots after milestone-audit per phase 44 D-01; graphify (order 54)
// slots after learnings(53) per phase 45 D-01; mempalace (order 55) slots after
// graphify(54) per phase 46 D-01; autonomous (out-of-band, not loop-ordered)
// slots after mempalace(55) per phase 49 D-01; repair (out-of-band, not
// loop-ordered) slots after gsdPhaseManagement per phase 58 D-01 — node-repair
// is an out-of-band ACTION, never a loop step.
...
const TABLE = {
  ...
  gsdOrient: {
    step: "orient",
    role: "orient",
    tools: ["gsd_init", "gsd_status", "gsd_progress", "gsd_new_milestone", "gsd_pause_work", "gsd_resume_work", "gsd_next", "gsd_route"],
    commands: ["gsd-init", "gsd-status", "gsd-progress", "gsd-new-milestone", "gsd-pause-work", "gsd-resume-work", "gsd-next", "gsd-route"],
  ...
  gsdQuickBatch: {
    step: "quick",
    role: "alternate",
    tools: ["gsd_quick_batch"],
    commands: ["gsd-quick-batch"],
    order: 25,
    prereq: [],
    next: [],
    produces: [],
  ...
  gsdMilestoneAudit: {
    step: "milestone-audit",
    role: "step",
    tools: ["gsd_milestone_audit"],
    commands: [],
    order: 52,
    prereq: [],
    next: [],
    produces: ["AUDIT.md"],
    consumes: ["VERIFICATION.md", "ROADMAP.md", "REQUIREMENTS.md"],
  },
  gsdLearnings: {
    step: "learnings",
    role: "step",
    tools: ["gsd_extract_learnings"],
    commands: ["gsd-extract-learnings"],
    order: 53,
    prereq: [],
    next: [],
    produces: ["LEARNINGS.md"],
    consumes: ["CONTEXT.md", "PLAN.md", "SUMMARY.md", "VERIFICATION.md", "REVIEW.md", "COVERAGE.md"],
  },
  gsdGraphify: {
    step: "graphify",
    role: "step",
    tools: ["gsd_graphify"],
    commands: ["gsd-graphify"],
    order: 54,
    prereq: [],
    next: [],
    produces: ["graph.json", "GRAPH_REPORT.md"],
    consumes: ["ROADMAP.md", "REQUIREMENTS.md", "STATE.md", "CONTEXT.md", "PLAN.md"],
  },
  gsdMempalace: {
    step: "mempalace",
    role: "step",
    tools: ["gsd_mempalace_recall", "gsd_mempalace_capture"],
    commands: ["gsd-mempalace-recall", "gsd-mempalace-capture"],
    order: 55,
    prereq: [],
    next: [],
    produces: ["MEMORY-RECALL.md"],
    consumes: ["CONTEXT.md", "PLAN.md", "SUMMARY.md"],
  },
  gsdAutonomous: {
    step: "autonomous",
    role: "out-of-band",
    tools: ["gsd_autonomous"],
    commands: ["gsd-autonomous"],
    order: NOT_LOOP_ORDERED,
    prereq: [],
    next: [],
    produces: ["VERIFICATION.md", "STATUS"],
    consumes: ["ROADMAP.md", "STATE.md", "CONTEXT.md"],
  },
  gsdAddTests: {
    step: "add-tests",
    role: "out-of-band",
    tools: ["gsd_add_tests"],
    commands: ["gsd-add-tests"],
    order: NOT_LOOP_ORDERED,
    prereq: [],
    next: [],
    produces: ["<NN>-ATEST.md", "TEST files"],
    consumes: ["SUMMARY.md", "CONTEXT.md", "VERIFICATION.md"],
  },
  gsdPhaseManagement: {
    step: "phase-management",
    role: "out-of-band",
    tools: ["gsd_phase"],
    commands: ["gsd-phase-manage"],
    order: NOT_LOOP_ORDERED,
    prereq: [],
    next: [],
    produces: [],
