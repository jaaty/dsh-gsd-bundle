...
export const GATE_DISPATCH = {
  ...
  broken_windows: {
    run: (d) => brokenWindowsGate(d.changedFiles, d.contentMap),
...
export function runCapabilityGates({ cfg, gitData, plans, skipGates = [] }) {
  ...
  const data = { changedFiles, contentMap, plans, commitSubjects };
  const reportLines = [];
  const failures = [];
...
// Integration tier: fetch the phase's changed files, their contents, and the
// branch's commit subjects through an injectable git wrapper (mirroring ship.js's
// git() helper — callers pass gitFn(cwd, argsArray)). Scopes to the merge-base
// diff so only the phase's changed files are scanned, never the whole repo (D-04).
// The content reads use node:fs/promises on a resolved absolute path; unreadable
// or deleted files are skipped.
export async function fetchGitData(cwd, gitFn, base) {
  const fs = await import("node:fs/promises");
  let defaultBranch = base;
  if (!defaultBranch) {
    const ref = gitFn(cwd, ["symbolic-ref", "refs/remotes/origin/HEAD", "--short"]).trim();
    defaultBranch = ref.replace(/^origin\//, "") || "main";
  }
  const mergeBase = gitFn(cwd, ["merge-base", "HEAD", defaultBranch]).trim();
  const changedFiles = mergeBase
    ? gitFn(cwd, ["diff", "--name-only", "--diff-filter=ACM", mergeBase, "HEAD"]).split("\n").filter(Boolean)
    : [];
  const contentMap = {};
  for (const file of changedFiles) {
    try {
  ...
  const commitSubjects = mergeBase
    ? gitFn(cwd, ["log", "--format=%s", `${mergeBase}..HEAD`]).split("\n").filter(Boolean)
    : [];
  return { changedFiles, contentMap, commitSubjects };
}
