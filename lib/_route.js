...
/**
 * Degrade a best-match command whose capability is absent (D-10). Routes to
 * the nearest present loop step (via effectiveRoutableStep) for loop-step
 * commands, or to gsd_status when gsdOrient is present, or to a present tool /
 * a generic orientation sentence otherwise. Never returns the absent command
 * (D-07) and never silently picks a different phase (D-10).
...
function degrade(command, descriptors) {
  ...
  const desc = cap ? buildCapability(cap) : null;
  const isLoopStep = desc && ["step", "optional", "alternate"].includes(desc.role);
  if (isLoopStep) {
    // The descriptor exposes a `step` field (verified against
    // lib/_capabilities.js buildCapability output, e.g. gsdDiscuss -> "discuss").
    const stepToken = desc.step ?? command.replace(/^gsd_/, "");
    const stepDesc = effectiveRoutableStep(`${stepToken}-phase`, descriptors);
    if (stepDesc) {
      const tool = buildCapability(stepDesc.key).tools[0];
      return {
        command: tool,
        note: `The ${command} command is unavailable (its ${cap} capability is absent); routing to the nearest available step ${tool}.`,
      };
    }
  }
  // Orient/out-of-band branch (or no loop step available).
