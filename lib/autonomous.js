...
// D-09: the gaps_found recovery loop is the SHARED repair helper (one-way
// import — the repair module never imports this driver; no cycle). The driver
// calls the helper, never the repair tool (P6: no recursion surface).
import { runRepairRounds, REPAIR_ROUND_BUDGET, BUDGET_EXHAUSTED_PHRASE } from "./repair.js";
...
export function buildAutoContext(phase, mode = "Auto-generated (discuss skipped — autonomous path)") {
  const n = phase?.n ?? 0;
...
// Drive one phase through its autopilot subagent (D-03). CONTEXT.md assurance
// is CALLER-OWNED (CR-12): runAutonomous's ensureAutoContext call right before
// this one is the single acquisition point — this function previously re-ran
// it, re-stating the artefact stat and always taking the { wrote: false } path
// while only obscuring which call owns the commit. Resolve the phase's artefact
// base, spawn exactly one fresh-context autopilot for the phase, and return its
// output on success. A thrown error from spawnSubagent (spawn/run failure) is
// caught and returned as { ok: false, step: "autopilot" } so the driver can
// record the hard failure (D-09). The autopilot itself invokes
...
async function runAutonomous(cwd, s, ctx, exec) {
  ...
  while (i < remaining.length) {
    ...
    // Success requires exactly status === "passed". A gaps_found status is no
    // longer an immediate stop: it first gets bounded automatic repair through
    // the shared repair-round helper (D-09); human_needed / missing /
    // unparseable statuses still stop immediately with no repair attempt —
    // human-owned items and unreadable reports are not machine-fixable plan
    // gaps (D-04).
    const { status } = await readVerifyStatus(cwd, s, phase.n);

    if (status === "gaps_found") {
      // Bounded automatic repair through the shared repair-round helper (the
      // same implementation the repair orchestrator exposes). NO rounds
      // override is passed, so the shared default budget of 2 applies
      // (D-03/D-07). Any thrown fault (a missing delegate step tool from the
      // helper's fail-loud guard, a repair-module error) is folded into a
      // hard stop with the real cause — never an uncaught throw (D-11).
      let repair = null;
      let repairFault = null;
      try {
        repair = await runRepairRounds({ cwd, s, ctx, exec, phaseNum: phase.n, phaseName: phase.name, gitFn: ctx.gitFn });
      } catch (e) {
        repairFault = e && e.message ? e.message : String(e);
      }
      // Re-read the status from the artefact — the helper's report string is
      // never the outcome oracle (artefact state is, mirroring the verify
      // readback discipline above).
      const post = await readVerifyStatus(cwd, s, phase.n);
      if (post.status === "passed") {
        // Recovered: continue the phase loop — the ROADMAP re-read at (4)
        // proceeds unchanged (D-09 continue-on-recovery).
        statuses.push({ number: phase.n, name: phase.name, status: `passed (recovered via ${repair ? repair.roundsRun : 0} repair round(s))` });
      } else if (repairFault != null) {
        statuses.push({ number: phase.n, name: phase.name, status: "failed" });
        stopReason = `Phase ${phase.n}: repair failed — ${repairFault}`;
        outcome = "stopped";
        break;
      } else {
        // Stop classification via the repair helper's designed contract, now a
        // shared SYMBOL (CR-05): its budget-exhaustion cause embeds the
        // BUDGET_EXHAUSTED_PHRASE constant imported from repair.js — matched
        // here by the same constant, so a wording change cannot silently
        // reclassify an early stop (whose cause must reach the human untouched
        // per D-11/P5, awaiting marker included) as budget exhaustion.
        const exhausted = repair != null && String(repair.stopReason ?? "").includes(BUDGET_EXHAUSTED_PHRASE);
        if (repair != null && repair.stopReason && !exhausted) {
          stopReason = `Phase ${phase.n}: repair stopped early — ${repair.stopReason}`;
        } else {
          // Budget genuinely exhausted (or a defensive null-cause non-passed
          // helper result): name the remaining gaps and the exhausted budget —
          // phase 49's D-09 final-stop semantics preserved (D-09). The closing
          // parenthetical keeps the same human-readable phrase for the report.
          stopReason = `Phase ${phase.n}: still "${post.status}" after ${repair ? repair.roundsRun : 0} repair round(s) (shared budget ${REPAIR_ROUND_BUDGET}) — remaining gaps in VERIFICATION.md (${BUDGET_EXHAUSTED_PHRASE})`;
        }
        statuses.push({ number: phase.n, name: phase.name, status: post.status });
        outcome = "stopped";
        break;
      }
    } else {
