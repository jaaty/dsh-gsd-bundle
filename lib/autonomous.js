...
import { runRepairRounds, REPAIR_ROUND_BUDGET } from "./repair.js";
  ...
  while (i < remaining.length) {
    ...
    if (status === "gaps_found") {
      ...
      if (post.status === "passed") {
        ...
        // Stop classification via the repair helper's designed string
        // contract: its budget-exhaustion cause carries the locked phrase
        // "repair budget exhausted"; every early-stop cause names its real
        // cause and never contains that phrase. An early stop is folded in
        // verbatim — any awaiting marker line inside it reaches the human
        // untouched so the existing answer/decision_id checkpoint-resume
        // handoff still works (D-11/P5) — and is never relabeled as budget
        // exhaustion.
        const exhausted = repair != null && String(repair.stopReason ?? "").includes("repair budget exhausted");
        if (repair != null && repair.stopReason && !exhausted) {
          ...
          // phase 49's D-09 final-stop semantics preserved (D-09).
          stopReason = `Phase ${phase.n}: still "${post.status}" after ${repair ? repair.roundsRun : 0} repair round(s) (shared budget ${REPAIR_ROUND_BUDGET}) — remaining gaps in VERIFICATION.md (repair budget exhausted)`;
        }
