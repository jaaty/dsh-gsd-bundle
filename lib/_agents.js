...
Write the file with the Write tool. Then return a one-paragraph structured summary: key findings, risks, and whether all Open Questions are (RESOLVED).`;
...
Each PLAN.md is written to <NN>-<PP>-PLAN.md with this YAML frontmatter:
  ...
  type: execute | tdd
  wave: <int>
  depends_on: ["<NN>-<PP>", ...]   # empty for wave 1
...
Worktree discipline (when run in an isolated git worktree):
...
After all tasks:
- Write <NN>-<PP>-SUMMARY.md with the Write tool (never heredoc). Frontmatter: phase, plan, subsystem, tags, dependency graph (requires/provides/affects), tech-stack, key-files (created/modified), decisions, metrics (duration, completed date), status: complete. Optional actuals: {tokens, tasks, commits}.
...
Return a short summary and the file path.`;
