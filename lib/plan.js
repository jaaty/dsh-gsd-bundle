...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // closed-phase gate
      if (await s.hasArtifact(cwd, args.phase, "VERIFICATION")) {
        const v = await s.readArtifact(cwd, args.phase, "VERIFICATION");
        if (/status:\s*passed/.test(v)) throw new Error(`gsd_plan: phase ${args.phase} already passed verification — replanning is blocked without --force`);
      }
