...
import { zeroPad, isClosedPhase, contextBudget } from "./_shared.js";
...
import { runAssumptionDeltaOnPlan } from "./assumption-delta.js";

const name = "gsd-plan";
const inject = ["gsdState", "tools", "subagents"];

// Auto-on-plan mempalace recall hook (D-07). A PURE, exported, directly-testable
// helper mirroring runLearningsOnShip in lib/ship.js: it takes only
// { cfg, tools, phase, exec } — NO ctx, NO git, NO gsdState. Gated by
// mempalace.enabled + recall_on_plan, it finds the registered gsd_mempalace_recall
// tool and invokes it with { phase }. Wrapped so a fault never blocks the plan
// step (onError: skip, REQ-MP-06). DEGR-05: if the mempalace plugin is retired
// (tool absent), the hook returns a not-registered/skipped line and never throws.
async function runMempalaceRecallOnPlan({ cfg, tools, phase, exec }) {
  if (cfg?.mempalace?.enabled !== true || cfg?.mempalace?.recall_on_plan !== true) return "mempalace recall: disabled (mempalace.enabled/recall_on_plan) — skipped";
  const tool = Array.isArray(tools) ? tools.find((t) => t && t.name === "gsd_mempalace_recall") : null;
  if (!tool) return "mempalace recall: gsd_mempalace_recall not registered — skipped";
  try {
    const r = await tool.execute({ phase }, exec);
    return "mempalace recall: " + String(r);
  } catch (e) {
    return "mempalace recall: recall failed (non-blocking): " + (e && e.message ? e.message : String(e));
  }
}

// Auto-on-plan mempalace capture hook (D-07). Same pure shape as the recall hook,
// gated by mempalace.enabled + capture_artifacts, invoking the registered
// gsd_mempalace_capture tool with { phase, artifact: "PLAN" } (OQ-3). Never
// blocks the plan step (onError: skip, REQ-MP-06).
async function runMempalaceCaptureOnPlan({ cfg, tools, phase, exec }) {
  ...
  try {
    const r = await tool.execute({ phase, artifact: "PLAN" }, exec);
...
function apply(ctx) {
  ...
  // revertible effect (D-09): retiring the plan plugin withdraws gsdPlan.
  ctx.provide("gsdPlan", buildCapability("gsdPlan"));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const maxBudget = contextBudget(cfg?.context_window);
      ...
      // D-07: mempalace recall at plan:pre (best-effort, never blocks the step —
      // runMempalaceRecallOnPlan catches any tool fault and returns a non-blocking
      // log line, REQ-MP-06). Gated by mempalace.enabled + recall_on_plan.
      log.push(await runMempalaceRecallOnPlan({ cfg, tools: ctx.tools, phase: args.phase, exec }));
      ...
      if (!args.skipResearch && (!hasResearch || args.forceResearch)) {
        ...
        if (pc.truncated.length) log.push(`planning-context: truncated ${pc.truncated.length} ${pc.truncated.length === 1 ? "entry" : "entries"} (${pc.truncated.map((t) => t.label).join(", ")}) — capping total context to ${maxBudget} chars`);
        const prompt = [
          RESEARCHER_PROMPT,
          pc.text,
      ...
      if (pc.truncated.length) log.push(`planning-context: truncated ${pc.truncated.length} ${pc.truncated.length === 1 ? "entry" : "entries"} (${pc.truncated.map((t) => t.label).join(", ")}) — capping total context to ${maxBudget} chars`);
      // D-05: assumption-delta plan:pre checkpoint — a PURE synchronous hook over
      // the already-fetched phase scope (ROADMAP goal + requirements + sealed
      // CONTEXT, D-02). When a signal is detected it splices a promote-vs-add-
      // alongside question block into the planner prompt and logs a line; it is
      // advisory and never advances STATE (D-08). The detector strips fenced code
      // blocks itself, so no stripping is needed here. Gated by
      // workflow.assumption_delta (D-04); a fault inside the hook cannot block the
      // plan step (D-08).
      const assumptionDelta = runAssumptionDeltaOnPlan({
        cfg,
        scopeText: [phase.goal, (phase.requirements || []).join(" "), contextMd].join("\n"),
      });
      const plannerPrompt = [
        PLANNER_PROMPT,
        pc.text,
        ...
        assumptionDelta.promptBlock ? assumptionDelta.promptBlock : "",
      ...
      if (assumptionDelta.logLine) log.push(assumptionDelta.logLine);
      ...
      // Gap-closure guard: the planner MUST produce a new fix plan (gap_closure: true).
      // The existing plans from the standard run are not enough — without a fix plan
      // gsd_execute --gaps-only would run nothing. Re-prompt once, then fail loud.
      if (args.gaps && !plans.some((p) => p.gap_closure)) {
        log.push("plan-checker: no gap_closure fix plan produced — re-prompting planner to write the fix plan file.");
        const fixPrompt = [
          PLANNER_PROMPT,
          `Gap-closure mode: you MUST write a NEW fix plan file at ${phaseDir}/${base}-04-PLAN.md (or the next free <PP>) with frontmatter gap_closure: true. The existing plans (${plans.map((p) => p.id).join(", ")}) are NOT fix plans. Use the Write tool to create the file on disk — do not just describe it.`,
          `\nReturn ## PLANNING COMPLETE when the fix plan file is written.`,
        ].join("\n\n");
        await spawnSubagent(ctx, exec, { label: `planner fix ${args.phase}`, promptText: fixPrompt });
        plans = await s.listPlans(cwd, args.phase);
        if (!plans.some((p) => p.gap_closure)) {
          return `gsd_plan: gap-closure mode but no fix plan (gap_closure: true) was produced in ${phaseDir} after re-prompting. gsd_execute --gaps-only would run nothing. Planner output:\n${plannerRes.output}`;
        }
      }
      let issues = await runChecker(ctx, exec, s, cwd, args.phase, plans, phase, contextMd, researchMd, reqs, log);
      let iter = 0;
      while (issues && iter < 3) {
        iter++;
        ...
        plans = await s.listPlans(cwd, args.phase);
        issues = await runChecker(ctx, exec, s, cwd, args.phase, plans, phase, contextMd, researchMd, reqs, log);
      }
      ...
      const commit = await commitArtifacts(cwd, args.phase, { scope: "plan", phaseName: phase.name });

      // D-07: mempalace capture at plan:post (best-effort, never blocks the step —
      // runMempalaceCaptureOnPlan catches any tool fault and returns a non-blocking
      // log line, REQ-MP-06). Files PLAN.md verbatim (OQ-3).
      log.push(await runMempalaceCaptureOnPlan({ cfg, tools: ctx.tools, phase: args.phase, exec }));
      ...
      return [
        ...
        `Planning artefacts committed: ${commit.committed} (${commit.staged.length} file(s)).${commit.warning ? " WARNING: " + commit.warning : ""}`,
  ...
  async function runChecker(ctx, exec, s, cwd, phaseNum, plans, phase, contextMd, researchMd, reqs, log) {
    const planContents = [];
...
export { name, inject, apply, runMempalaceRecallOnPlan, runMempalaceCaptureOnPlan };