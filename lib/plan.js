  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (!args.force && await s.hasArtifact(cwd, args.phase, "VERIFICATION")) {
        ...
        if (/status:\s*passed/.test(v)) throw new Error(`gsd_plan: phase ${args.phase} already passed verification. Re-run with force=true to replan (clears the closed-phase gate).`);
      }
