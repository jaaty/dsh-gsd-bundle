...
const COMMANDS = [
  ...
  {
    name: "gsd-health",
    description: "Health check phase N: inspect .planning/ integrity and offer non-destructive repair (dry-run by default; --repair applies config-only fixes).",
    hint: "<N> [--repair]",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-health <N> [--repair]" };
      const repair = /--repair/.test(raw);
      return {
        text: "Run the gsd_health tool on phase " + n + (repair ? " with repair:true to apply config-only non-destructive fixes" : " (dry-run — no writes, will report the .planning/ integrity status)") + ".",
        ack: "Health check phase " + n + " → gsd_health.",
      };
    },
  },
  {
  ...
  },
  {
    ...
    build: (raw) => {
      const parts = raw
        ...
        .map((p) => p.trim())
        .filter(Boolean);
      if (parts.length === 0) return { err: "Usage: /gsd-quick-batch <task1> | <task2> | ..." };
      return { text: `Run the gsd_quick_batch tool with these tasks: ${JSON.stringify(parts)}`, ack: "Quick batch → gsd_quick_batch." };
    },
  },
  {
    name: "gsd-fast-mode",
    description: "Fast mode phase N: lightweight single-pass fast path for a simple phase (auto-CONTEXT -> one executor -> SUMMARY -> lightweight verify -> full ship).",
    hint: "<N>",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-fast-mode <N>" };
      return {
        text: `Run the gsd_fast_mode tool on phase ${n}: a lightweight single-pass fast path that auto-derives a minimal CONTEXT, runs the phase through one fresh-context executor, records a SUMMARY, performs a lightweight verify, and ships the phase the full way.`,
  ...
  {
    name: "gsd-mvp-phase",
    description: "MVP phase N: interactive propose-then-confirm scoping -> real PLAN.md (via gsd_plan) -> delegate to the normal loop (execute/verify/ship).",
    hint: "<N>",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-mvp-phase <N>" };
      return {
        text: `Run the gsd_mvp_phase tool on phase ${n}: it proposes a minimal-viable slice of the phase goal, asks you to confirm or adjust it, then produces a real PLAN.md (via gsd_plan) and delegates execution to the normal loop (gsd_execute -> gsd_verify -> gsd_ship).`,
        ack: `MVP phase ${n} → gsd_mvp_phase.`,
      };
    },
  },
  {
  ...
  },
  {
    ...
    description: "Extract learnings from phase N: accumulate decisions, lessons, patterns, and surprises into a carrying-forward LEARNINGS.md.",
    hint: "<N> [--force]",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-extract-learnings <N> [--force]" };
      const force = /--force/.test(raw);
      return {
        text: "Run the gsd_extract_learnings tool on phase " + n + (force ? " with force:true to re-extract" : "") + ".",
        ack: "Extract learnings phase " + n + " → gsd_extract_learnings.",
      };
    },
  },
  {
    name: "gsd-graphify",
    description: "Build, query, or inspect the project knowledge graph in .planning/graphs/.",
    hint: "build|query <term>|status",
    build: (raw) => {
      const m = raw.trim().match(/^(build|status)$/);
      if (m) return { text: "Run the gsd_graphify tool with action " + m[1] + ".", ack: "Graphify " + m[1] + " → gsd_graphify." };
      const q = raw.trim().match(/^query\s+(.+)$/);
      if (q) return { text: "Run the gsd_graphify tool to query the graph for: " + q[1].trim(), ack: "Graphify query → gsd_graphify." };
      return { err: "Usage: /gsd-graphify build|status|query <term>" };
    },
  },
  {
    name: "gsd-mempalace-recall",
    description: "Deliberate recall before discuss/plan — produces MEMORY-RECALL.md from the MemPalace CLI.",
    hint: "<N>",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-mempalace-recall <N>" };
      return { text: "Run the gsd_mempalace_recall tool on phase " + n + ".", ack: "Mempalace recall phase " + n + " → gsd_mempalace_recall." };
    },
  },
  {
    name: "gsd-mempalace-capture",
    description: "File CONTEXT/PLAN/SUMMARY verbatim into the palace at phase boundaries.",
    hint: "<N> <CONTEXT|PLAN|SUMMARY>",
    build: (raw) => {
      const n = phaseNum(raw);
      const artifact = raw.trim().match(/\b(CONTEXT|PLAN|SUMMARY)\b/);
      if (!n || !artifact) return { err: "Usage: /gsd-mempalace-capture <N> <CONTEXT|PLAN|SUMMARY>" };
      return { text: "Run the gsd_mempalace_capture tool on phase " + n + " artifact " + artifact[1] + ".", ack: "Mempalace capture phase " + n + " → gsd_mempalace_capture." };
    },
  },
  {
  ...
  {
    name: "gsd-pause-work",
    description: "Pause work mid-phase: write a structured context handoff (HANDOFF.json + .continue-here.md) and commit it as a WIP commit.",
    build: () => ({
      text: "Run the gsd_pause_work tool to write a structured context handoff (HANDOFF.json + .continue-here.md) and commit it as a WIP commit.",
      ack: "Pausing work → gsd_pause_work.",
    }),
  },
  {
    name: "gsd-resume-work",
    description: "Resume work from a handoff or incomplete-work detection: present status + next-action and update Session Continuity.",
    build: () => ({
      text: "Run the gsd_resume_work tool to restore full context from the handoff (or detect incomplete work) and present the status + next-action.",
      ack: "Resuming work → gsd_resume_work.",
    }),
  },
  {
    name: "gsd-next",
    description: "Detect the current project state and route to the best next action; with auto-advance, re-point STATE to the next logical step and return the command to invoke.",
    build: () => ({
      text: "Run the gsd_next tool to classify the current .planning/ state and route to the best next action. When it re-points STATE (auto-advance), report the command to invoke next but do not run it.",
      ack: "Smart entry → gsd_next.",
    }),
  },
  {
    name: "gsd-route",
    ...
    hint: "<intent>",
    build: (raw) => {
      if (!raw.trim()) return { err: "Usage: /gsd-route <intent>" };
      return { text: "Run the gsd_route tool with this intent: " + raw.trim(), ack: "Routing intent → gsd_route." };
    },
  },
  {
    name: "gsd-autonomous",
    description: "Drive all remaining incomplete phases of the active milestone autonomously (discuss/plan/execute/verify per phase, no ship, no lifecycle).",
    build: () => ({
      text: "Run the gsd_autonomous tool to drive every remaining incomplete phase of the active milestone through discuss/plan/execute/verify without per-phase prompting.",
      ack: "Autonomous → gsd_autonomous.",
    }),
  },
  {
    name: "gsd-add-tests",
    description: "Add unit and Integration tests for a completed phase from its UAT criteria and implementation (gsd_add_tests).",
    hint: "[phase]",
    build: (raw) => {
      const n = phaseNum(raw);
      return {
        text: `Run the gsd_add_tests tool${n ? ` with phase ${n}` : ""} to create unit and Integration tests for a completed phase from its SUMMARY/CONTEXT/VERIFICATION and implementation. It is advisory and never advances STATE.`,
        ack: "Adding tests via gsd_add_tests.",
      };
    },
  },
  {
    name: "gsd-repair",
    description: "Node repair phase N: bounded automatic recovery rounds (gsd_plan --gaps → gsd_execute --gaps-only → gsd_verify --gaps) for a phase whose verification found gaps (gsd_repair).",
    hint: "<N> [--rounds 1|2]",
    build: (raw) => {
      const n = phaseNum(raw);
      if (!n) return { err: "Usage: /gsd-repair <N> [--rounds 1|2]" };
      const rounds = (raw.match(/--rounds\s+(\d+)/) || [])[1];
      return {
        text: `Run the gsd_repair tool on phase ${n}${rounds ? ` with rounds ${rounds}` : ""}: it runs bounded automatic recovery rounds of gsd_plan --gaps → gsd_execute --gaps-only → gsd_verify --gaps for a gaps_found verification, writes <NN>-REPAIR.md, and never advances STATE itself. Long-running: each round spawns planner/checker/executor/verifier subagents.`,
        ack: `Repair phase ${n} → gsd_repair.`,
      };
    },
  },
  {
    name: "gsd-phase-manage",
    description: "Add, insert, remove, reorder, or edit phases directly in ROADMAP.md with validation and integrity checks (gsd_phase).",
    hint: "add|insert|remove|reorder|edit ...",
    build: (raw) => {
      const action = String(raw || "").trim().split(/\s+/)[0];
      if (!["add", "insert", "remove", "reorder", "edit"].includes(action)) {
        return { err: "Usage: /gsd-phase-manage add|insert|remove|reorder|edit ..." };
      }
      return {
        text: `Run the gsd_phase tool with action ${action} and the parsed flags (name/goal/requirements/at/n/to/status/yes) from: ${raw.trim()}`,
        ack: `Phase management ${action} → gsd_phase.`,
      };
    },
  },
];
