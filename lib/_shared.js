...
export function parseRequirements(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    // ID grammar: uppercase segments joined by hyphens, ending in digits —
    // CLH-08, SHIP-01, but also multi-segment IDs like SHIP-CLEAN-01. The
    // former single-hyphen pattern ([A-Z]+-\d+) silently failed to parse
    // multi-segment IDs, so a phase referencing them failed phase-management's
    // fail-closed validation even with the requirement block present.
    const m = line.match(/^[-*]\s*\[([ x])\]\s*([A-Z]+(?:-[A-Z]+)*-\d+)\s*[:：]\s*(.+)$/i);
...
// Dual-shape registered-tools lookup (CR-03). The offline mount/test harness
// passes ctx.tools as an ARRAY (with a .register method); the real DSH runtime
// exposes a SERVICE object answering .get(name). The one-shape array-only
// pattern silently skipped every capture/learnings/graphify hook in the
// production runtime, so every tool-seam consumer routes through this helper
// and the two shapes cannot drift again. Returns undefined when neither shape
// matches. (Same contract as lib/quick.js:43-47 / lib/repair.js, now
// single-sourced here — _shared.js is a leaf module so this is cycle-safe.)
export function findToolIn(tools, toolName) {
  if (Array.isArray(tools)) return tools.find((t) => t && t.name === toolName);
  if (tools && typeof tools.get === "function") return tools.get(toolName);
  return undefined;
}

// Parse D-NN decision entries from a CONTEXT.md body. Matches the exact
// discuss.js format `- **D-NN:** text` (lib/discuss.js:138). The closing `:**`
// bounds the digits so D-01 never matches inside D-010 (whole-ID safety).
// Plain bullets without a D-ID (`- some note`, `- **Claude's Discretion:** …`)
// are NOT matched — only lines that are exactly `- **D-NN:** …`. Pure and
// I/O-free: the single source of truth for CONTEXT decision parsing, shared by
// gap-analysis's parseDecisionIds and gsd_ship's Key-Decisions render. Returns
// an array of { id, text } objects, deduplicated by id and sorted ascending by
// the numeric part of the id (mirrors gap-analysis's ordering).
export function parseDecisionEntries(md) {
  ...
  while ((m = re.exec(String(md ?? ""))) !== null) {
    const id = m[1];
    const text = m[2].trim();
    if (!byId.has(id)) byId.set(id, text); // first occurrence wins
  }
  return [...byId.entries()]
    .map(([id, text]) => ({ id, text }))
...
// Parse a git `--name-status -z` diff stream into structured entries. Relocated
// verbatim from lib/_clean-branch.js (phase 51, D-03) so lib/undo.js keeps
// working after that module is deleted. Pure and I/O-free.
//
// The status letter (token[0]) decides the entry shape: R consumes TWO path
// tokens, every other status exactly ONE. A rename's similarity score rides in
// the SAME first token (`R100`), so the rename branch MUST match
// `token.startsWith("R")` and NEVER `token === "R"` — a scored `R100` token
// treated as a one-path entry would desynchronize the entire parse and swallow
// the following old/new path tokens.
