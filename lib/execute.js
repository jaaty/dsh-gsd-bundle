...
import { zeroPad, matchesGapClosure, nowIso, resolvePlanDep, contextBudget } from "./_shared.js";
import { prepareCheckpoint, processCheckpoint } from "./_checkpoint.js";
...
function apply(ctx) {
  ...
  // revertible effect (D-09): retiring the execute plugin withdraws gsdExecute.
  ctx.provide("gsdExecute", buildCapability("gsdExecute"));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      for (const [w, wavePlans] of waves) {
        // skip plans blocked by unfinished deps (reuse the planIndex runnable set)
        const runnable = wavePlans.filter((p) => idx.runnable.includes(p));
        ...
        const runnables = await Promise.all(runnable.map(async (p) => {
          ...
          const prep = await prepareCheckpoint(s, { cwd, phase: args.phase, p, answer: args.answer, decisionId: args.decision_id });
          if (prep.awaiting) return { p, awaiting: true, marker: prep.marker };
          const resumeInstr = prep.resumeInstr;
        ...
        const results = await Promise.all(runnables.filter((r) => !r.awaiting).map(async ({ p, thunk, job }) => {
          ...
          if (cp && typeof cp === "object") {
            return await processCheckpoint(s, { cwd, phase: args.phase, p, r, job, log, w });
          }
      ...
      const commit = await commitArtifacts(cwd, args.phase, { scope: "execute", phaseName: phase.name });
      log.push(`Planning artefacts committed: ${commit.committed} (${commit.staged.length} file(s)).${commit.warning ? " WARNING: " + commit.warning : ""}`);
      return log.join("\n");
    },
