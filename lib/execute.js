...
import { zeroPad, matchesGapClosure, nowIso, parseFrontmatter, stringifyFrontmatter, resolvePlanDep } from "./_shared.js";
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      for (const [w, wavePlans] of waves) {
        ...
        const results = await Promise.all(runnables.map(async ({ p, thunk, job }) => {
          const r = await thunk();
