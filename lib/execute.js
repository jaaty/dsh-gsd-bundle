    ...
    async execute(args, exec) {
      ...
      for (const [w, wavePlans] of waves) {
        ...
        const runnables = await Promise.all(runnable.map(async (p) => {
          ...
          // D-03/D-04/D-06: not awaiting — an answer is available, either from
          // this call's matching answer+decision_id or from a persisted
          // human_answer. Bind it into the resume instruction and, when this call
          // supplied a matching answer, persist it so a context-reset resume
          // carries it (D-04). A stale/non-matching decision_id never reaches this
          // block (awaiting would be true) — it is ignored, no error (D-06).
          if (checkpointFm) {
            const storedDecisionId = checkpointFm.decision_id || decisionIdFor(p.id, checkpointFm.last_completed_task);
            const suppliedAnswer = typeof args.answer === "string" && args.answer.trim() !== "";
            const suppliedMatches = suppliedAnswer
              && typeof args.decision_id === "string"
              && typeof checkpointFm.decision_id === "string"
              && args.decision_id === checkpointFm.decision_id;
            // D-04: persist a this-call answer into the CHECKPOINT frontmatter
            // (route through s.writeArtifact — DUR-06, never raw node:fs).
            if (suppliedMatches) {
              const cpText = await s.readArtifact(cwd, args.phase, cpSuffix);
              const { frontmatter } = parseFrontmatter(cpText);
              frontmatter.human_answer = args.answer;
              await s.writeArtifact(cwd, args.phase, cpSuffix, stringifyFrontmatter(frontmatter));
            }
            // The answer text: this call's answer when it supplied a matching one,
            // else the persisted human_answer (context-reset case, D-04).
            const answer = suppliedMatches ? args.answer : (checkpointFm.human_answer || "");
            resumeInstr += `\nRESUME from checkpoint: human answered ${storedDecisionId} = ${answer}`;
          }
          const prompt = [
            EXECUTOR_PROMPT,
            planningContext([
        ...
        const results = await Promise.all(runnables.filter((r) => !r.awaiting).map(async ({ p, thunk, job }) => {
          ...
          if (cp && typeof cp === "object") {
            ...
            await s.writeArtifact(cwd, args.phase, `CHECKPOINT-${zeroPad(Number(p.plan))}`, stringifyFrontmatter({
              ...
              // D-01/D-07: persist the decision kind so the awaiting marker can
              // name it; D-03/RQ-3: persist a deterministic decision_id so a
              // turn-2 answer can be matched exactly (never misapplied).
