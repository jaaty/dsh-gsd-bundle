...
// directly). It commits the map with "docs(planning): codebase map" when it can.
//
...
import { CODEBASE_MAPPER_PROMPT, CODEBASE_QUERY_PROMPT, GSD_INTEL_UPDATER_PROMPT } from "./_agents.js";
import { buildManifest, compareManifest, clampConfidence, changedFilesToDocs } from "./_intel.js";
...
// The seven map-document names (the fixed FOCUS_DOCS universe). gsd_intel_updater
// filters changedFilesToDocs candidates down to these before rewriting (D-05:
// never invent a doc, and never rewrite a map doc that has no existing file).
const VALID_DOC_NAMES = ["STACK", "INTEGRATIONS", "ARCHITECTURE", "STRUCTURE", "CONVENTIONS", "TESTING", "CONCERNS"];
...
function apply(ctx) {
  ...
  // effect (D-09): retiring the map-codebase plugin withdraws gsdMapCodebase.
  ctx.provide("gsdMapCodebase", buildCapability("gsdMapCodebase"));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // commit the codebase map (best-effort) via the shared seam (D-11/D-12):
      // phaseNum null + a message override because map has no phase.
      const committed = (await commitArtifacts(cwd, null, { scope: "map", message: "docs(planning): codebase map" })).committed;
      ...
      const summary = [
        ...
        committed ? "Committed: docs(planning): codebase map." : "Note: codebase map written but not committed (no git repo, nothing staged, or git unavailable).",
        "",
        "## Next Up",
        projectExists
  ...
  // gsd-intel-updater (CBQX-02/D-04): targeted re-map of the codebase map.
  // Registered alongside gsd_map_codebase in the SAME plugin (D-04) so no
  // cordis.patch.yml row, package.json export, or insert-count changes. Given
  // drifted repo-relative paths (or auto-detected from the map manifest), it
  // maps them to the affected .planning/codebase/ docs and rewrites ONLY those
  // via a fresh-context per-doc updater subagent, leaving unrelated docs
  // untouched (D-05). Returns the same structured object contract as the map
  // tool (CBQX-03/D-06).
  ctx.tools.register(defineTool({
    name: "gsd_intel_updater",
    description: "Targeted re-map of the codebase map (opengsd gsd-intel-updater): given drifted repo-relative paths (or auto-detected from the stored .planning/codebase/.map-manifest.json), update ONLY the affected map documents via a fresh-context updater subagent, leaving unrelated documents untouched. Omit paths to auto-detect drifted paths from the manifest.",
    parameters: {
      paths: { type: "array", items: { type: "string" }, description: "Drifted repo-relative paths. Omit to auto-detect drifted paths from the map manifest." },
    },
    output: {
      schema: {
        type: "object",
    ...
    async execute(args, exec) {
      const cwd = cwdOf(exec);
      const s = gsd();
      if (!s) throw new Error("gsd_intel_updater: gsdState service unavailable");

      // Drifted paths: explicit paths win (validated like the map tool); else
      // auto-detect from the stored manifest by diffing the live tree (D-04).
      const manifest = await s.readCodebaseManifest(cwd);
      let driftedPaths = [];
      if (args.paths && args.paths.length) {
        driftedPaths = validatePaths(args.paths);
      } else if (Array.isArray(manifest) && manifest.length) {
        const current = buildManifest(await walkRepo(ctx, s, cwd));
        const drift = compareManifest(manifest, current);
        driftedPaths = [...drift.added, ...drift.removed, ...drift.modified];
      }

      if (!driftedPaths.length) {
        if (!Array.isArray(manifest) || !manifest.length) {
          return { kind: "notice", text: "No .planning/codebase/.map-manifest.json found — run gsd_map_codebase (force=true) to build the map snapshot, then retry gsd_intel_updater." };
        }
        return { kind: "notice", text: "No drift detected since the last map." };
      }

      // CBQX-02/D-05: map the drifted paths to the affected docs via the
      // heuristic rule table (changedFilesToDocs), then keep only those that
      // are valid map docs AND currently exist — never invent or rewrite a doc
      // that has no current file to preserve. Unrelated docs are left untouched.
      const docs = changedFilesToDocs(driftedPaths)
        .filter((d) => VALID_DOC_NAMES.includes(d));
      const existingDocs = [];
      for (const d of docs) {
        const txt = await s.readCodebaseDoc(cwd, `${d}.md`);
        if (txt !== undefined && txt !== null) existingDocs.push(d);
      }
      if (!existingDocs.length) {
        return { kind: "notice", text: "No affected map documents to update." };
      }

      // Fresh-context per-doc updater subagent (D-04/D-05): reads ONLY the
      // listed existing docs, explores ONLY the drifted paths, rewrites ONLY
      // the listed docs, and never touches unrelated docs.
      const date = today();
      const codebaseDir = s.codebaseDir(cwd);
      const prompt = [
        `Today's date: ${date}`,
        `Map directory: ${codebaseDir}/`,
        `Affected documents:`,
        ...existingDocs.map((d) => `- ${d}.md`),
        `Drifted paths:`,
        ...driftedPaths.map((p) => `- \`${p}\``),
        "Read ONLY the listed existing documents. Explore ONLY the drifted paths above. Rewrite ONLY the listed documents (per-doc, targeted, template-faithful), preserving the map's structure and leaving ALL unrelated documents byte-identical and untouched. Set all date stamps to the Today's date line.",
        "",
        GSD_INTEL_UPDATER_PROMPT,
      ].join("\n");
      const r = await spawnSubagent(ctx, exec, { label: "gsd-intel-updater", promptText: prompt });

      return {
        kind: "updater",
        text: "Targeted codebase update complete.\n\nAffected documents rewritten:\n"
          + existingDocs.map((d) => `- ${d}.md`).join("\n")
          + (r.output ? "\n\n" + String(r.output).slice(0, 300) : ""),
        docs: existingDocs,
      };
    },
    presentCall: (a) => ({ card: "generic", title: "Update codebase intel", kind: "other", rawInput: { paths: (a.paths || []).length } }),
  }));
}
