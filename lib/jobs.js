...
export function createJobsRuntime() {
  const live = new Map();
  // Best-effort teardown invoked on unload/HMR of the owning fiber (DEGR-06
  // D-03). Cancels every running job: aborts subagent controllers, kills shell
  // children, clears timeout timers, and best-effort writes 'cancelled' to the
  // persisted async-jobs manifest for each. NEVER deletes subagent entries
  // (D-02 — the subagent `.then` remains the only removal point) and NEVER
  // throws (D-03 — unload may be tearing down gsdState).
  async function cancelAll() {
    for (const [id, rec] of live) {
      if (rec.kind === "subagent") {
        rec.handle.cancelled = true;
        rec.handle.controller.abort();
        if (rec.handle.timer) clearTimeout(rec.handle.timer);
      } else {
        try { rec.handle.kill(); } catch { /* already gone */ }
      }
      try {
        await rec.s.updateJob(rec.cwd, id, {
          status: "failed",
          reason: { reason: "cancelled", detail: "cancelled on unload" },
        });
      } catch { /* best-effort — unload may be tearing down gsdState (D-03) */ }
    }
  }
  return { live, cancelAll };
}
...
async function startRun(runtime, ctx, s, cwd, entry, jobsCfg) {
  if (entry.kind === "subagent") {
    ...
    child.unref();
    runtime.live.set(entry.id, { kind: "shell", handle: child, s, cwd });
...
async function startSubagentRun(runtime, ctx, s, cwd, entry) {
  ...
  const rec = { controller: ac, timedOut: false, cancelled: false, dispose: run.dispose, timer: null };
  runtime.live.set(entry.id, { kind: "subagent", handle: rec, s, cwd });
  await s.updateJob(cwd, entry.id, { status: "running", started: nowIso() });
