...
export async function launchJob(ctx, s, cwd, opts = {}) {
...
async function startRun(ctx, s, cwd, entry, jobsCfg) {
  if (entry.kind === "subagent") {
    await startSubagentRun(ctx, s, cwd, entry);
  } else {
    ...
    child.unref();
    live.set(entry.id, { kind: "shell", handle: child });
...
async function startSubagentRun(ctx, s, cwd, entry) {
  ...
  live.set(entry.id, { kind: "subagent", handle: rec });
  await s.updateJob(cwd, entry.id, { status: "running", started: nowIso() });
...
export async function cancelJob(ctx, s, cwd, id) {
  const { entries } = await s.readJobs(cwd);
...
export async function scheduleJobs(ctx, s, cwd) {
  try {
    const { entries } = await s.readJobs(cwd);
    const jobsCfg = resolveJobsConfig(await s.readConfig(cwd));
    ...
    for (const entry of entries) {
      ...
      await startRun(ctx, s, cwd, entry, jobsCfg);
      runningCount += 1;
    }
  } catch {
...
export async function retryJob(ctx, s, cwd, id, opts = {}) {
  const { entries } = await s.readJobs(cwd);
  const entry = entries.find((e) => e.id === id);
  if (!entry) return { ok: false, message: `job ${id} not found` };
