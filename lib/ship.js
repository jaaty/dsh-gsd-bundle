...
// Auto-on-ship mempalace capture hook (D-07, OQ-3). A PURE, exported,
// directly-testable helper mirroring runLearningsOnShip / runGraphifyOnShip: it
// takes only { cfg, tools, phase, exec } — NO ctx, NO git, NO gsdState. Gated by
// mempalace.enabled + capture_artifacts, it finds the registered
// gsd_mempalace_capture tool and invokes it with { phase, artifact: "SUMMARY" }
// (re-files the final SUMMARY.md into the milestones room, OQ-3). Wrapped so a
// fault never blocks the ship (onError: skip, REQ-MP-06). DEGR-05: if the
// mempalace plugin is retired (tool absent), the hook returns a
// not-registered/skipped line and never throws.
async function runMempalaceCaptureOnShip({ cfg, tools, phase, exec }) {
  if (cfg?.mempalace?.enabled !== true || cfg?.mempalace?.capture_artifacts !== true) return "mempalace capture: disabled (mempalace.enabled/capture_artifacts) — skipped";
  const tool = Array.isArray(tools) ? tools.find((t) => t && t.name === "gsd_mempalace_capture") : null;
  if (!tool) return "mempalace capture: gsd_mempalace_capture not registered — skipped";
  try {
    const r = await tool.execute({ phase, artifact: "SUMMARY" }, exec);
    return "mempalace capture: " + String(r);
  } catch (e) {
    return "mempalace capture: capture failed (non-blocking): " + (e && e.message ? e.message : String(e));
  }
}
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // ── 10.7 auto-on-ship mempalace capture hook (D-07, OQ-3) ────────────────
      // Best-effort verbatim capture of the just-shipped phase's final SUMMARY.md
      // into the palace's milestones room, gated by mempalace.enabled +
      // capture_artifacts. Runs AFTER the completion commit + push so it fires for
      // the final state. Never blocks the ship — runMempalaceCaptureOnShip catches
      // any tool fault and returns a non-blocking log line (REQ-MP-06).
      const mempalaceLine = await runMempalaceCaptureOnShip({ cfg, tools: ctx.tools, phase: args.phase, exec });
      log.push(mempalaceLine);
...
export { name, inject, apply, preflightError, runLearningsOnShip, runGraphifyOnShip, runMempalaceCaptureOnShip };