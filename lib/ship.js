...
import { parseFrontmatter, zeroPad, isValidRef, parseDecisionEntries, findToolIn } from "./_shared.js";
...
async function runLearningsOnShip({ cfg, tools, phase, exec }) {
  ...
  const tool = findToolIn(tools, "gsd_extract_learnings");
...
async function runGraphifyOnShip({ cfg, tools, exec }) {
  ...
  const tool = findToolIn(tools, "gsd_graphify");
...
async function runMempalaceCaptureOnShip({ cfg, tools, phase, exec }) {
  ...
  const tool = findToolIn(tools, "gsd_mempalace_capture");
