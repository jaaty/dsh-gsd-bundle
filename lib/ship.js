...
import { parseFrontmatter, zeroPad, isValidRef, parseDecisionEntries } from "./_shared.js";
...
// Auto-on-ship learnings hook (D-10). A PURE, exported, directly-testable helper
// (mirrors the preflightError precedent): it takes only { cfg, tools, phase,
// exec } — NO ctx, NO git, NO gsdState — so it is unit-testable offline with a
// fake tools array. Gated by workflow.learnings (default false), it finds the
// registered gsd_extract_learnings tool and invokes it with force:true (the
// just-shipped phase may already be in phases_extracted from a prior manual run;
// D-06 force override re-extracts the final state). Wrapped so a fault never
// blocks the ship: a throw is caught and surfaced as a non-blocking log line
// with the real cause. The tool's own execute already commits via
// commitArtifacts (D-11) and degrades to decisions-only on subagent fault
// (D-09), so never-block is satisfied end-to-end. DEGR-05: if the learnings
// plugin is retired (tool absent), the hook returns a not-registered/skipped
// line and never throws.
async function runLearningsOnShip({ cfg, tools, phase, exec }) {
  if (!cfg?.workflow?.learnings) return "learnings: disabled (workflow.learnings false) — skipped";
  const tool = Array.isArray(tools) ? tools.find((t) => t && t.name === "gsd_extract_learnings") : null;
  if (!tool) return "learnings: gsd_extract_learnings not registered — skipped";
  try {
    const r = await tool.execute({ phase, force: true }, exec);
    return "learnings: " + String(r);
  } catch (e) {
    return "learnings: extraction failed (non-blocking): " + (e && e.message ? e.message : String(e));
  }
}

// Auto-on-ship graphify hook (D-08). A PURE, exported, directly-testable helper
// mirroring runLearningsOnShip, but with NO phase param because the graph is
// project-global (D-04/D-08). It takes only { cfg, tools, exec } — NO ctx, NO
// git, NO gsdState — so it is unit-testable offline with a fake tools array.
// Gated by workflow.graphify (default false), it finds the registered
// gsd_graphify tool and invokes it with { action: 'build' } (the auto-run
// rebuilds the project-global graph wholesale from the current .planning/ state,
// D-04 idempotent rebuild). Wrapped so a fault never blocks the ship: a throw is
// caught and surfaced as a non-blocking log line with the real cause. The tool's
// own execute already commits via commitArtifacts (D-09) and preserves the prior
// graph on failure (D-09), so never-block is satisfied end-to-end. DEGR-05: if
// the graphify plugin is retired (tool absent), the hook returns a
// not-registered/skipped line and never throws.
async function runGraphifyOnShip({ cfg, tools, exec }) {
  if (!cfg?.workflow?.graphify) return "graphify: disabled (workflow.graphify false) — skipped";
  const tool = Array.isArray(tools) ? tools.find((t) => t && t.name === "gsd_graphify") : null;
  if (!tool) return "graphify: gsd_graphify not registered — skipped";
  try {
    const r = await tool.execute({ action: "build" }, exec);
    return "graphify: " + String(r);
  } catch (e) {
    return "graphify: build failed (non-blocking): " + (e && e.message ? e.message : String(e));
  }
}
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // ── 5.7 clean-PR branch (GAP-01, D-01…D-09) ─────────────────────────────────
      // Built only AFTER the capability gates (5.5) and the pre-ship-verify gate
      // (5.6) pass, and BEFORE the push (6). Since Phase 43 (option C) the clean
      // branch carries the FULL phase record — real code + the per-phase
      // `.planning/phases/` planning subtree — so that `main` becomes the durable
      // record after the clean PR merges and the phase branch can be deleted without
      // losing the planning memory. buildCleanBranch (plan 01) switches back to the
      // current (phase-N) branch when it finishes, so the working tree is on phase-N
      // throughout, and the completion-state commit (step 10) lands on phase-N and is
      // then cherry-picked onto the clean branch (D-03/R1/OQ-2).
      let prBranch = branch; // phase-N default; the PR head becomes the clean branch when built.
      if (cleanPr) {
        try {
          const info = await buildCleanBranch({
            cwd,
            gitFn: git,
            phaseNum: args.phase,
          ...
          if (info.built) {
            prBranch = info.cleanBranch;
            log.push(`clean branch ${prBranch} ready (squash of non-.planning/phases changes)`);
          } else {
            // D-07: a planning-/doc-only phase changes no real files → ship phase-N as-is.
            log.push(`clean branch skipped: ${info.reason}; shipping phase branch as-is`);
          }
        } catch (e) {
          fail(`clean-PR branch construction failed: ${e.message}`, e);
        }
      }
      ...
      // D-05: both the phase-N source-of-truth branch and (when a real code change
      // exists) the clean presentation branch are pushed.
      if (cleanPr && prBranch !== branch) {
        try {
          await git(cwd, ["push", "-u", "origin", prBranch]);
          log.push(`pushed clean branch ${prBranch}`);
        } catch (e) {
          fail(`git push of clean branch failed: ${e.message}`, e);
        }
      }
      ...
      // Key Decisions: list ONLY the decisions sealed in THIS phase's CONTEXT.md
      // (`- **D-NN:** text` entries), not the project-wide state.body.decisions
      // ledger (which every addDecision appends to forever — including each
      // ship's "Phase N shipped — PR #X"). Missing CONTEXT or no decisions →
      // (none); never fall back to the global ledger.
      const ctxText = await s.readArtifact(cwd, args.phase, "CONTEXT").catch(() => "");
      const decisions = parseDecisionEntries(ctxText);
      ...
      const body = [
        ...
        "### Key Decisions",
        "",
        ...(decisions.length ? decisions.map((d) => `- **${d.id}:** ${d.text}`) : ["(none)"]),
        "",
      ...
      try {
        ...
        if (prBranch) prArgs.push("--head", prBranch);
      ...
      if (staged) {
        try {
          ...
          // Option C: when a clean-PR branch was built, the PR head is the clean
          // branch, not phase-N. The completion-state commit above landed on phase-N
          // only, so the clean branch (and therefore main after merge) would still be
          // missing the "phase complete" markers. Cherry-pick the same commit onto the
          // clean branch and push it so main becomes the durable record and the phase
          // branch can be deleted safely.
          if (cleanPr && prBranch !== branch) {
            const completionCommit = (await git(cwd, ["rev-parse", "HEAD"])).trim();
            await git(cwd, ["switch", prBranch]);
            try {
              await git(cwd, ["cherry-pick", completionCommit]);
              await git(cwd, ["push", "origin", prBranch]);
              log.push(`pushed completion state to clean branch ${prBranch}`);
            } catch (e) {
              await git(cwd, ["cherry-pick", "--abort"]).catch(() => {});
              fail(`applying completion state to clean branch failed: ${e.message}`, e);
            } finally {
      ...
      // ── 10.5 auto-on-ship learnings hook (D-10) ──────────────────────────────
      // Best-effort learnings extraction for the just-shipped phase, gated by
      // workflow.learnings. Runs AFTER the completion commit + push so it fires
      // for the final state. Never blocks the ship — runLearningsOnShip catches
      // any tool fault and returns a non-blocking log line (D-10). force:true
      // re-extracts even if the phase is already in phases_extracted (D-06).
      const learningsLine = await runLearningsOnShip({ cfg, tools: ctx.tools, phase: args.phase, exec });
      log.push(learningsLine);

      // ── 10.6 auto-on-ship graphify hook (D-08) ──────────────────────────────
      // Best-effort rebuild of the project-global knowledge graph for the
      // just-shipped phase, gated by workflow.graphify. Runs AFTER the completion
      // commit + push so it fires for the final state. Never blocks the ship —
      // runGraphifyOnShip catches any tool fault and returns a non-blocking log
      // line (D-08). The graph is project-global, so no phase param (D-04/D-08).
...
export { name, inject, apply, preflightError, runLearningsOnShip, runGraphifyOnShip };