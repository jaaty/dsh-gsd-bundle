...
import { parseFrontmatter, zeroPad } from "./_shared.js";
...
function gh(cwd, args) {
  return execSync(`gh ${args}`, { cwd, encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] }).trim();
}
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        prUrl = gh(cwd, `pr create --title ${JSON.stringify(title)} --body-file ${JSON.stringify(tmp)} --base ${defaultBranch} ${draft}`);
      } catch (e) {
