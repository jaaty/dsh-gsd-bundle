...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // (5.6) pass, and BEFORE the push (6), so the phase PR leaves reviewers with
      // only real code changes plus the durable cross-phase artefacts — the
      // per-phase `.planning/phases/` subtree is excluded (D-01/D-02). buildCleanBranch
      // (plan 01) switches back to the current (phase-N) branch when it finishes, so
      // the working tree is on phase-N throughout, and the completion-state commit
      // (step 10) still lands on phase-N, never the clean branch (D-03/R1/OQ-2).
