...
// Auto-on-discuss mempalace recall hook (D-07). A PURE, exported,
// directly-testable helper mirroring runLearningsOnShip in lib/ship.js: it
// takes only { cfg, tools, phase, exec } — NO ctx, NO git, NO gsdState — so it
// is unit-testable offline with a fake tools array. Gated by mempalace.enabled +
// recall_on_discuss, it finds the registered gsd_mempalace_recall tool and
// invokes it with { phase }. Wrapped so a fault never blocks the discuss step
// (onError: skip, REQ-MP-06). DEGR-05: if the mempalace plugin is retired (tool
// absent), the hook returns a not-registered/skipped line and never throws.
async function runMempalaceRecallOnDiscuss({ cfg, tools, phase, exec }) {
  if (cfg?.mempalace?.enabled !== true || cfg?.mempalace?.recall_on_discuss !== true) return "mempalace recall: disabled (mempalace.enabled/recall_on_discuss) — skipped";
...
// Auto-on-discuss mempalace capture hook (D-07). Same pure shape as the recall
// hook, gated by mempalace.enabled + capture_artifacts, invoking the registered
// gsd_mempalace_capture tool with { phase, artifact: "CONTEXT" } (OQ-3). Never
// blocks the discuss step (onError: skip, REQ-MP-06).
async function runMempalaceCaptureOnDiscuss({ cfg, tools, phase, exec }) {
  ...
  try {
    const r = await tool.execute({ phase, artifact: "CONTEXT" }, exec);
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // D-07: mempalace recall at discuss:pre (best-effort, never blocks the
      // step — runMempalaceRecallOnDiscuss catches any tool fault and returns a
      // non-blocking log line, REQ-MP-06). Gated by mempalace.enabled +
      // recall_on_discuss.
      log.push(await runMempalaceRecallOnDiscuss({ cfg, tools: ctx.tools, phase: args.phase, exec }));
      ...
      // D-07: mempalace capture at discuss:post (best-effort, never blocks the
      // step — runMempalaceCaptureOnDiscuss catches any tool fault and returns a
      // non-blocking log line, REQ-MP-06). Files CONTEXT.md verbatim (OQ-3).
      log.push(await runMempalaceCaptureOnDiscuss({ cfg, tools: ctx.tools, phase: args.phase, exec }));
      ...
      return [
        `Discuss complete for phase ${args.phase} (${phase.name}). Wrote ${ctxPath}. STATE advanced to 'plan'.${specGuidance}${branchNote}${commitNote} Next: gsd_plan on phase ${args.phase}.`,
        ...log,
      ].join("\n");
    },
...
export { name, inject, apply, runMempalaceRecallOnDiscuss, runMempalaceCaptureOnDiscuss };