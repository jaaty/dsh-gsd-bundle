...
import { nowIso, parseFrontmatter } from "./_shared.js";
...
import { classifyNextState, renderNextRecommendation } from "./_next.js";
import { classifyIntent, renderRouteRecommendation } from "./_route.js";
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      // throws "without inject". Guard so the gather falls back to the
      // default git seam (mirrors lib/code-review.js) while still honouring
      // a host-provided gitFn when present.
  ...
  // ── gsd_next ─────────────────────────────────────────────────────────────────
  // Smart-entry routing + auto-advance surface (phase 53, CLH-02/CLH-03). The
  // execute path gathers a state snapshot via the gsdState accessors, calls the
  // pure classifyNextState (lib/_next.js), applies the branch's optional STATE
  // mutation (branch 5 only, guarded by the advance flag — Task 2), and returns
  // renderNextRecommendation text. Auto-advance re-points STATE but does NOT
  // auto-run the next step's tool (D-02): the returned text names the command to
  // invoke next.
  ctx.tools.register(defineTool({
    name: "gsd_next",
    description: "Detect the current project state and route the user to the best next action, with an auto-advance option (opengsd /gsd-next).",
    parameters: {
      advance: { type: "boolean", description: "When true, re-point STATE to the next logical step for the advance branches and return the command to invoke (does not auto-run it)." },
    },
    output: { schema: { type: "string" }, render: (_a, v) => [{ type: "text", text: v }] },
    async execute(args, exec) {
      const cwd = cwdOf(exec);
      const s = gsd();
      if (!s) throw new Error("gsd_next: gsdState service unavailable");

      // Gather the snapshot (D-03): the execute path does the I/O; the pure
      // classifier consumes it. hasProject distinguishes branch 1 (no
      // .planning/ project at all) from branch 2 (.planning/ exists but
      // STATE/ROADMAP missing/corrupt) per OQ-1.
      let project, state, roadmap, handoff, continueHere, milestoneAudit;
      project = await s.readProject(cwd).catch(() => undefined);
      state = await s.readState(cwd).catch(() => undefined);
      roadmap = await s.readRoadmap(cwd).catch(() => undefined);
      handoff = await s.readHandoff(cwd).catch(() => undefined);

      // Paused-handoff detection (D-05): read the .continue-here.md pointer
      // fully, mirroring gsd_resume_work's continueHereFiles scan. The root
      // pointer is read first; when absent, every phase dir is scanned so a
      // lone phase-dir .continue-here.md (with HANDOFF.json deleted) still
      // routes to gsd_resume_work rather than misrouting to mid-phase.
      continueHere = await s.readContinueHere(cwd, "").catch(() => undefined);
      if (continueHere === undefined) {
        try {
          const dirs = await s.listPhaseDirs(cwd);
          for (const d of dirs) {
            const ch = await s.readContinueHere(cwd, d.name).catch(() => undefined);
            if (ch !== undefined) { continueHere = ch; break; }
          }
        } catch { /* listPhaseDirs unavailable — treat as no pointer */ }
      }

      // Milestone audit read-back (D-09): when a roadmap exists, read the
      // milestone-scoped audit and parse its frontmatter status via the canonical
      // parseFrontmatter helper (lib/_shared.js — the single shared YAML-subset
      // scanner state.js/milestone-audit.js/verify.js already use; no parallel
      // scanner inlined here). Tolerate a missing audit (undefined ->
      // not-yet-audited -> branch 6 routes to gsd_milestone_audit).
      if (roadmap) {
        const milestoneName = roadmap.milestoneName || (state && state.frontmatter && state.frontmatter.milestone_name) || "milestone";
        const auditText = await s.readMilestoneArtifact(cwd, milestoneName).catch(() => undefined);
        if (auditText !== undefined) {
          try {
            const { frontmatter: afm } = parseFrontmatter(String(auditText));
            if (afm && afm.status) milestoneAudit = { status: String(afm.status).trim() };
          } catch { /* corrupt audit — treat as not-yet-audited */ }
        }
      }

      const hasProject = !!(project || state || roadmap);
      const snapshot = { hasProject, state, roadmap, handoff, continueHere, milestoneAudit };

      // Capability descriptors (D-06): routing is capability-aware so smart-entry
      // never recommends a step whose tool/capability is absent.
      const descriptors = availableCapabilities((k) => ctx.get(k));

      // D-03 ordering: classify-then-mutate-then-render. The pure classifier
      // decides the branch + recommendation + optional mutation descriptor; the
      // execute path applies the mutation (if any) and renders the text.
      const result = classifyNextState(snapshot, descriptors);

      // D-02: auto-advance re-points STATE but does NOT auto-run the next step's
      // tool — the returned text names the command to invoke. Only branch 5
      // (PHASE_SHIPPED_NEXT) carries a non-null mutation; branches 1,2,3,4,6
      // return mutation: null and must NEVER mutate STATE or commit (the
      // never-advances-STATE invariant).
      if (result.mutation && result.mutation.setActivePhase) {
        const { phaseNum, step } = result.mutation.setActivePhase;
        if (args && args.advance) {
          // Re-point STATE (active_phase + status + recomputed next_action) via
          // the existing mutator, then commit through the atomic-commit seam.
          // Mirrors gsd_new_milestone (NO ensurePhaseBranch, per OQ-4). Never
          // throws on a commit warning — commitArtifacts returns { warning? }
          // instead of throwing.
          await s.setActivePhase(cwd, phaseNum, step);
          // gitFn is not a host-provided service; in an active fiber ctx.gitFn
          // throws "without inject". Guard so the advance commit falls back to
          // the default git seam (mirrors lib/code-review.js) while still
          // honouring a host-provided gitFn when present.
          let gitFn = defaultGitFn;
          try { gitFn = ctx.gitFn || defaultGitFn; } catch { /* not injected — use default */ }
          await commitArtifacts(cwd, phaseNum, { scope: "next", phaseName: String(phaseNum) }, gitFn);
          return renderNextRecommendation(result);
        }
        // Mutation available but advance not requested: name the command and
        // note that re-running with advance:true will re-point STATE — but do
        // NOT mutate.
        return `${renderNextRecommendation(result)}\n(Re-run gsd_next with advance:true to re-point STATE to phase ${phaseNum} (${step}).)`;
      }

      // No mutation (branches 1,2,3,4,6): render unchanged, STATE untouched.
      return renderNextRecommendation(result);
    },
    presentCall: (a) => ({ card: "generic", title: "gsd_next", kind: "other", rawInput: { advance: a.advance } }),
  }));

  // ── gsd_route ───────────────────────────────────────────────────────────────
  // Freeform-routing surface (phase 54, CLH-04). The execute path parses a
  // plain-English intent string and dispatches it to the most appropriate
  // available GSD command. It is recommend-only (D-02): it classifies the intent
  // via the pure classifyIntent (lib/_route.js) and returns the rendered
  // recommendation text — it NEVER auto-runs the routed step tool and NEVER
  // reads/mutates STATE/ROADMAP (out-of-scope). It needs only ctx (for
  // availableCapabilities) — no gsdState, no cwd.
  ctx.tools.register(defineTool({
    name: "gsd_route",
    ...
    parameters: {
      intent: { type: "string", description: "Plain-English intent to route, e.g. 'discuss phase 3'." },
    },
    output: { schema: { type: "string" }, render: (_a, v) => [{ type: "text", text: v }] },
    async execute(args, exec) {
      // Capability descriptors (D-06): routing is capability-aware so the router
      // never recommends a step whose tool/capability is absent.
      const descriptors = availableCapabilities((k) => ctx.get(k));
      const result = classifyIntent(args.intent, descriptors);
      return renderRouteRecommendation(result);
    },
