...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // milestone-scoped audit and parse its frontmatter status. Tolerate a
      // missing audit (undefined -> not-yet-audited). Task 2 refines this parse
      // to use the canonical parseFrontmatter helper.
      if (roadmap) {
        ...
        if (auditText !== undefined) {
          const m = String(auditText).match(/^---\r?\n[\s\S]*?\r?\nstatus:\s*(\S+)/);
          if (m) milestoneAudit = { status: m[1].trim() };
        }
      }
      ...
      // Task 1 (tracer): classify + render only; auto-advance mutation is added
      // in Task 2. The never-advances-STATE invariant holds for every branch
      // here because no mutation is applied regardless of args.advance.
