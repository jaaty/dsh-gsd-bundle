...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // recompute progress totals (phases + completed + percent) from the roadmap
      const st = await s.readState(cwd);
      st.frontmatter.progress.total_phases = roadmap.phases.length;
      const done = roadmap.phases.filter((p) => p.status === "Complete").length;
      st.frontmatter.progress.completed_phases = done;
      st.frontmatter.progress.percent = roadmap.phases.length ? Math.round((done / roadmap.phases.length) * 100) : 0;
      await s.writeState(cwd, st);
