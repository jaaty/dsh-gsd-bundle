...
const INFO_ROLES = ["orient", "jobs", "onboarding", "out-of-band"];
...
const NEXT_ACTION_TO_STEP = new Map([
  ...
  ["ui-phase", "ui"],
  ["plan-phase", "plan"],
  ["gap-analysis-phase", "gap-analysis"],
...
const STEP_PARAGRAPHS = {
  ...
  gsdGapAnalysis:
    "- Gap-analysis: after planning, emit a deterministic coverage table cross-referencing every phase REQ-ID and CONTEXT D-ID against the runnable plans' bodies (frontmatter + prose). A soft gate flags uncovered IDs in a committed COVERAGE.md without blocking the next step.",
  gsdExecute:
  ...
  gsdCodeReview:
    "- Code review: after execute, review the phase's changed source files for bugs, security issues, and quality defects. A fresh-context reviewer subagent produces REVIEW.md with severity-classified findings (BLOCKER/WARNING/INFO). An optional --fix companion applies findings with per-fix atomic commits into REVIEW-FIX.md. Soft gate \u2014 advisory, never blocks verify or ship.",
  gsdUiReview:
    "- Ui-review: after execute, a retroactive 6-pillar UI audit reviews the phase's implemented frontend code against the UI-SPEC (or abstract 6-pillar standards). A fresh-context gsd-ui-auditor subagent scores 6 pillars (Copywriting, Visuals, Color, Typography, Spacing, Experience Design) 1-4 each (overall /24), classifies findings BLOCKER/WARNING, and writes UI-REVIEW.md. Soft gate \u2014 advisory, never blocks verify or ship.",
  gsdVerify:
  ...
  gsdMilestoneAudit:
    "- Milestone-audit: at milestone-close, a deterministic close-gate aggregates per-phase verifications to confirm the milestone met its definition of done, plus a cross-phase UAT outstanding-items list, emitted as one milestone-scoped audit report. Soft gate \u2014 advisory, never blocks the release.",
  gsdLearnings:
    "- Learnings: after a phase completes, extract decisions, lessons, patterns, and surprises from its planning artefacts into a carrying-forward LEARNINGS.md that accumulates across phases. Soft gate \u2014 advisory, never blocks ship or the next phase.",
  gsdGraphify:
    "- Graphify: build a project knowledge graph in .planning/graphs/ from a deterministic pure-JS scan of the planning artefacts, with a tool to build, query, and inspect it. Soft gate \u2014 advisory, never blocks ship or the next phase.",
  gsdQuick:
