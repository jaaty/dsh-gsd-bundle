...
var GsdState = class {
  ...
  // Read the codebase-map drift manifest at .planning/codebase/.map-manifest.json.
  // Returns the parsed array when present and well-formed, else null. Tolerant
  // like readConfig/readWindows: a missing file OR a corrupt JSON payload yields
  // null (never throws) — the caller treats null as "no prior manifest".
  async readCodebaseManifest(cwd) {
    const text = await this._read(`${this.codebaseDir(cwd)}/.map-manifest.json`);
  ...
  // Persist the drift manifest at .planning/codebase/.map-manifest.json. MUST
  // route through this._write → ctx.fs (DUR-06 / D-01): the manifest is a
  // .planning/ artefact and is committed with the map, never written with raw
  // node:fs/promises.
  async writeCodebaseManifest(cwd, manifest) {
    await this._write(`${this.codebaseDir(cwd)}/.map-manifest.json`, JSON.stringify(manifest, null, 2) + "\n");
  }
  ...
  // ── pause/resume data tier (phase 48, GAP-14) ───────────────────────────────
  // The gsd_pause_work / gsd_resume_work tools read/write the HANDOFF.json +
  // .continue-here.md handoff set and update Session Continuity. All writes route
  // through this._write → ctx.fs (DUR-06), never raw node:fs/promises; all reads
  // through this._read. Missing/corrupt inputs degrade, never throw (D-09).

  // List the per-phase directories under .planning/phases/ as [{ name, mtime }].
  // Absent phases dir → []. mtime is null when the host fs exposes none (FakeFs).
  async listPhaseDirs(cwd) {
    const dir = this._phases(cwd);
    const target = await this.ctx.fs.resolve(dir);
    const stat = await this.ctx.fs.stat(target);
    if (!stat) return [];
    const entries = await this.ctx.fs.listDir(target);
    ...
    for (const e of entries) {
      ...
      const s = await this.ctx.fs.stat(e.target);
      out.push({ name: e.name, mtime: s && s.mtime != null ? s.mtime : null });
    }
    return out;
  }

  // Update Session Continuity (D-04). recordSession does NOT set resumeFile, so
  // this is the accessor resume-work uses to stamp stoppedAt + resumeFile.
  async updateContinuity(cwd, { stoppedAt, resumeFile }) {
    const doc = (await this.readState(cwd)) || this._freshState();
    doc.body.continuity.stoppedAt = stoppedAt ?? null;
    doc.body.continuity.resumeFile = resumeFile ?? null;
    doc.frontmatter.stopped_at = stoppedAt ?? null;
    return this.writeState(cwd, doc);
  }

  // Read .planning/HANDOFF.json → parsed object, or undefined when absent or
  // corrupt (degrade, D-09).
  async readHandoff(cwd) {
    const text = await this._read(`${this._planning(cwd)}/HANDOFF.json`);
    if (text === undefined) return undefined;
    try { return JSON.parse(text); } catch { return undefined; }
  }

  async writeHandoff(cwd, handoff) {
    const file = `${this._planning(cwd)}/HANDOFF.json`;
    await this._write(file, `${JSON.stringify(handoff, null, 2)}\n`);
    return file;
  }

  // One-shot HANDOFF lifecycle (D-05): delete after a successful resume consumes
  // it. Absent file is a no-op — routed through ctx.fs.unlink (DUR-06) so it works
  // over both the real fs and the FakeFs test adapter.
  async deleteHandoff(cwd) {
    const file = `${this._planning(cwd)}/HANDOFF.json`;
    const target = await this.ctx.fs.resolve(file);
    try { await this.ctx.fs.unlink(target); } catch { /* absent — nothing to remove */ }
  }

  // Read the .continue-here.md pointer at the phase-dir path when phaseDir is a
  // non-empty string, else at the .planning/ root (D-02 / D-03).
  async readContinueHere(cwd, phaseDir) {
    const file = phaseDir
      ? `${this._phases(cwd)}/${phaseDir}/.continue-here.md`
      : `${this._planning(cwd)}/.continue-here.md`;
    return this._read(file);
  }

  async writeContinueHere(cwd, phaseDir, content) {
    const file = phaseDir
      ? `${this._phases(cwd)}/${phaseDir}/.continue-here.md`
      : `${this._planning(cwd)}/.continue-here.md`;
  ...
  _nextActionFor(step) {
    return { spec: "spec-phase", discuss: "discuss-phase", ui: "ui-phase", plan: "plan-phase", "gap-analysis": "gap-analysis-phase", execute: "execute-phase", review: "verify-phase", "ui-review": "verify-phase", verify: "verify-phase", validate: "ship-phase", ship: "ship-phase", done: null }[step] || "discuss-phase";
  }
  ...
  // Raw config accessor for the health diagnostic (OQ-3). readConfig silently
  // degrades a missing OR corrupt config.json to the defaults, which masks the
  // three distinct states health must distinguish (missing / unparseable /
  // missing-keys). readConfigRaw returns the RAW text (or undefined when absent)
  // WITHOUT JSON.parse and WITHOUT falling back to defaults, so the caller can
  // tell them apart. defaultConfig() exposes the canonical _defaultConfig({})
  // shape as the single-source schema for validation and repair.
  async readConfigRaw(cwd) {
    return this._read(`${this._planning(cwd)}/config.json`);
  }
  ...
  // ── milestone-scoped artefacts (DUR-06) ─────────────────────────────────────
  // The milestone-close audit report lives at .planning/milestones/<name>-AUDIT.md
  // (phase 43 D-06). Unlike the per-phase writeArtifact, this is a milestone-scoped
  // (not phase-scoped) artefact, so it needs its own accessors. Both route through
  // this._write → ctx.fs (never raw node:fs/promises); _write → _ensureParent
  // creates the .planning/milestones/ dir (no-throw) before writing.
  milestoneDir(cwd) { return `${this._planning(cwd)}/milestones`; }

  async writeMilestoneArtifact(cwd, milestoneName, content) {
    const file = `${this.milestoneDir(cwd)}/${slugify(milestoneName)}-AUDIT.md`;
    await this._write(file, content);
    return file;
  }

  async readMilestoneArtifact(cwd, milestoneName) {
    return this._read(`${this.milestoneDir(cwd)}/${slugify(milestoneName)}-AUDIT.md`);
  }

  // ── root-scoped carrying-forward LEARNINGS.md (DUR-06 / phase 44 D-04) ──────
  // The carrying-forward .planning/LEARNINGS.md is project-scoped (NOT
  // phase-scoped), accumulating every phase's extract across the project. Unlike
  // writeArtifact (which resolves into the per-phase dir), this needs its own
  // root-scoped accessors modeled on writeMilestoneArtifact. Both route through
  // this._write/_read → ctx.fs (never raw node:fs/promises); _write →
  // _ensureParent creates the .planning/ dir (no-throw) before writing.
  async writeRootLearnings(cwd, content) {
    const file = `${this._planning(cwd)}/LEARNINGS.md`;
    await this._write(file, content);
    return file;
  }

  async readRootLearnings(cwd) {
    return this._read(`${this._planning(cwd)}/LEARNINGS.md`);
  }

  // ── project-scoped .planning/graphs/ (DUR-06 / phase 45 D-06) ────────────────
  // The project knowledge graph lives at .planning/graphs/ (graph.json +
  // GRAPH_REPORT.md). It is project-global (NOT phase-scoped), so it needs its
  // own accessors modeled on writeMilestoneArtifact / writeRootLearnings. Both
  // route through this._write/_read → ctx.fs (never raw node:fs/promises);
  // _write → _ensureParent creates the .planning/graphs/ dir (no-throw) before
  // writing.
  graphsDir(cwd) { return `${this._planning(cwd)}/graphs`; }

  async writeGraphArtifact(cwd, name, content) {
    const file = `${this.graphsDir(cwd)}/${name}`;
    await this._write(file, content);
    return file;
  }

  async readGraphArtifact(cwd, name) {
    return this._read(`${this.graphsDir(cwd)}/${name}`);
  }

  async hasGraphArtifact(cwd, name) {
    const t = await this.ctx.fs.resolve(`${this.graphsDir(cwd)}/${name}`);
    return !!(await this.ctx.fs.stat(t));
  }

  // ── project-scoped .planning/.mempalace-stage/ (DUR-06 / phase 46 D-06) ──────
  // The MemPalace capture staging tree lives at .planning/.mempalace-stage/,
  // project-global (NOT phase-scoped), so it needs its own accessors modeled on
  // writeGraphArtifact / writeRootLearnings. All route through this._write/_read
  // → ctx.fs (never raw node:fs/promises); _write → _ensureParent creates the
  // parent dirs (no-throw) before writing.
  mempalaceStageDir(cwd) { return `${this._planning(cwd)}/.mempalace-stage`; }

  async writeMempalaceStage(cwd, relPath, content) {
    const file = `${this.mempalaceStageDir(cwd)}/${relPath}`;
    await this._write(file, content);
    return file;
  }

  async readMempalaceStage(cwd, relPath) {
    return this._read(`${this.mempalaceStageDir(cwd)}/${relPath}`);
  }

  async hasMempalaceStage(cwd, relPath) {
    const t = await this.ctx.fs.resolve(`${this.mempalaceStageDir(cwd)}/${relPath}`);
    return !!(await this.ctx.fs.stat(t));
  }

  // Newest mtime among the .planning/ artefacts (D-07). Thin I/O feeding the
  // pure computeStaleness helper: recursively walks .planning/ via ctx.fs.listDir
  // and returns the max numeric mtime among files that expose one, or null when
  // none do (e.g. FakeFs stat returns no mtime → null). Not FakeFs-testable.
  async newestPlanningMtime(cwd) {
    const root = this._planning(cwd);
    let max = null;
    const walk = async (dir) => {
      ...
      if (!stat) return;
      if (stat.type === "file") {
        if (stat.mtime != null && (max === null || stat.mtime > max)) max = stat.mtime;
        return;
      }
      const entries = await this.ctx.fs.listDir(target);
      for (const e of entries) {
        if (e.type === "file") {
          const s = await this.ctx.fs.stat(e.target);
          if (s && s.mtime != null && (max === null || s.mtime > max)) max = s.mtime;
        } else {
          await walk(e.target.targetKey);
        }
      }
    };
    await walk(root);
  ...
  // Reconcile the loop cursor to idle. Used by the milestone-close audit when it
  // determines the milestone is ready-to-close: every phase is Complete, so no
  // phase should be active. This clears a stale cursor (e.g. one left pointing at
  // a phase that was re-verified during the audit pass) without touching progress
  // counters or the roadmap. It is NOT setActivePhase — it never advances to a
  // phase step; it only resets to the idle baseline.
  async reconcileToIdle(cwd) {
    const doc = await this.readState(cwd);
    if (!doc) return;
    doc.frontmatter.status = "idle";
    doc.frontmatter.active_phase = null;
    doc.frontmatter.next_action = null;
    doc.frontmatter.next_phases = [];
    doc.frontmatter.current_phase = null;
    doc.frontmatter.current_phase_name = null;
    doc.frontmatter.current_plan = null;
    return this.writeState(cwd, doc);
  }
