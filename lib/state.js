...
var GsdState = class {
  ...
  async writeArtifact(cwd, phaseNum, suffix, content) {
    ...
    const name = await this._phaseDirName(cwd, phaseNum);
    // suffix is like "CONTEXT", "RESEARCH", "VERIFICATION", "UAT", "UI-SPEC",
    // or a plan/summary "PLAN-01" / "SUMMARY-01".
    const file = `${dir}/${name.split("/").pop()}-${suffix}.md`;
  ...
  async readArtifact(cwd, phaseNum, suffix) {
    const dir = await this.phaseDir(cwd, phaseNum);
    const name = await this._phaseDirName(cwd, phaseNum);
    return this._read(`${dir}/${name.split("/").pop()}-${suffix}.md`);
  }

  async hasArtifact(cwd, phaseNum, suffix) {
    const dir = await this.phaseDir(cwd, phaseNum);
    const name = await this._phaseDirName(cwd, phaseNum);
    const t = await this.ctx.fs.resolve(`${dir}/${name.split("/").pop()}-${suffix}.md`);
    return !!(await this.ctx.fs.stat(t));
  }
