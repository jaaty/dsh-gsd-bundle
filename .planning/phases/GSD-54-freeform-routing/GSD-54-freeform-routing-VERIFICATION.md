...
**23/23 must-haves verified** (9 truths + 9 artifacts + 5 key-links, all VERIFIED/WIRED). No FAILED truths, no MISSING/STUB artifacts, no NOT_WIRED links, no blocker anti-patterns, no human-verification items.
...
| `lib/core-tools.js` | ✓ | ✓ `gsd_route` tool registration (lines 680-695) | ✓ under gsdOrient |
...
- **Missing-phase (D-09):** `classifyIntent("discuss", full)` → `{command:"gsd_discuss", missingArg:"phase"}` — never fabricates a phase. ✓
