...
- [x] GAP-12: A cross-session memory integration performs deliberate recall before discuss/plan and verbatim capture at phase boundaries (mempalace).
...
- [x] CLH-02: Smart entry: detect the current project state and route the user to the best next action.
...
Backfilled 2026-09-08 (integrity repair, gsd_phase fail-closed validation):
these definitions were present when phase 51 (drop-clean-branch) was added and
discussed (see its DISCUSSION-LOG) but were dropped from this file by a later
milestone rewrite — restored verbatim from the phase 51 CONTEXT specifics, all
shipped by phase 51 (PR #54).

- [x] SHIP-CLEAN-01: gsd_ship no longer builds or pushes a phase-NN-clean branch; the PR head is the phase-NN branch directly.
- [x] SHIP-CLEAN-02: the clean-branch helper module is removed after its shared parsing helper is relocated for remaining consumers.
- [x] SHIP-CLEAN-03: the clean_pr_branch config surface and health-check repair expectations are removed.
- [x] SHIP-CLEAN-04: all clean-branch tests removed/updated and the full suite passes.
