...
describe("loopSteps / informationEntries ordering (D-08)", () => {
  ...
  test("informationEntries returns orient|jobs|onboarding|out-of-band in CAPABILITY_KEYS position", () => {
    ...
    // (out-of-band) then gsdAutonomous (out-of-band) then gsdAddTests,
    // gsdPhaseManagement and gsdRepair (out-of-band, last in CAPABILITY_KEYS).
    assert.deepEqual(keys, ["gsdMapCodebase", "gsdOrient", "gsdJobs", "gsdUndo", "gsdHealth", "gsdAutonomous", "gsdAddTests", "gsdPhaseManagement", "gsdRepair"]);
  });
