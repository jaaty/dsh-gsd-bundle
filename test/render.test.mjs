...
// mempalace 55.
const LOOP_ORDER = ["gsdSpec", "gsdDiscuss", "gsdUi", "gsdPlan", "gsdGapAnalysis", "gsdQuick", "gsdQuickBatch", "gsdExecute", "gsdCodeReview", "gsdUiReview", "gsdVerify", "gsdValidatePhase", "gsdShip", "gsdMilestoneAudit", "gsdLearnings", "gsdGraphify", "gsdMempalace"];
...
describe("loopSteps / informationEntries ordering (D-08)", () => {
  ...
  test("informationEntries returns orient|jobs|onboarding|out-of-band in CAPABILITY_KEYS position", () => {
    ...
    // map-codebase (index 0) then orient (1) then jobs (2); gsdUndo and gsdHealth
    // (out-of-band) then gsdAutonomous (out-of-band) then gsdAddTests and
    // gsdPhaseManagement (out-of-band, last in CAPABILITY_KEYS).
    assert.deepEqual(keys, ["gsdMapCodebase", "gsdOrient", "gsdJobs", "gsdUndo", "gsdHealth", "gsdAutonomous", "gsdAddTests", "gsdPhaseManagement"]);
  });
  ...
  test("loopSteps excludes a removed step and keeps ascending order", () => {
    ...
    assert.deepEqual(
      ...
      ["gsdSpec", "gsdDiscuss", "gsdUi", "gsdPlan", "gsdGapAnalysis", "gsdQuick", "gsdQuickBatch", "gsdExecute", "gsdCodeReview", "gsdUiReview", "gsdValidatePhase", "gsdShip", "gsdMilestoneAudit", "gsdLearnings", "gsdGraphify", "gsdMempalace"],
    );
  });
