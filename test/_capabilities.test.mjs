...
describe("capability key surface (DEGR-01)", () => {
  test("exposes exactly the 28 known keys", () => {
    assert.equal(CAPABILITY_KEYS.length, 28);
    for (const key of [
      "gsdOrient",
      "gsdJobs",
      "gsdSpec",
...
describe("capability mapping (D-04)", () => {
  test("gsdOrient tools + commands are exact", () => {
    ...
    assert.deepEqual(d.tools, ["gsd_init", "gsd_status", "gsd_progress", "gsd_new_milestone", "gsd_pause_work", "gsd_resume_work", "gsd_next", "gsd_route"]);
    assert.deepEqual(d.commands, ["gsd-init", "gsd-status", "gsd-progress", "gsd-new-milestone", "gsd-pause-work", "gsd-resume-work", "gsd-next", "gsd-route"]);
  });
