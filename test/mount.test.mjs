...
// Apply all 15 plugins in patch order against a ctx; throw with the offending id.
...
// Expected registered tool names (17) — verified against the real modules.
const EXPECTED_TOOL_NAMES = [
  "gsd_init", "gsd_status", "gsd_progress", "gsd_new_milestone",
  "gsd_discuss", "gsd_spec_phase", "gsd_plan", "gsd_gap_analysis",
  "gsd_execute", "gsd_code_review", "gsd_verify", "gsd_ship", "gsd_ui_phase",
...
describe("mount: all 15 plugins activate", () => {
  let fs, ctx;
  beforeEach(() => {
    fs = new FakeFs();
    // Supply subagents so the gsd_job sub-fiber activates (DEGR-07 D-05) and
    // the full 15-tool surface is registered.
    ctx = makeMountCtx(fs, { subagents: makeSubagents() });
  });

  test("applies all 15 plugins in patch order without throwing", async () => {
    await applyAll(ctx);
    ...
    assert.ok(ctx.tools.length === 17, `expected 17 tools, got ${ctx.tools.length}`);
    assert.ok(ctx.commands.length === 15, `expected 15 commands, got ${ctx.commands.length}`);
    ...
    // DEGR-01: all 13 capability services are provided with the documented
    ...
    assert.ok(CAPABILITY_KEYS.length === 13, `expected 13 capability keys, got ${CAPABILITY_KEYS.length}`);
  ...
  test("absent capability leaves its slash command unregistered (DEGR-03)", async () => {
    // Apply every plugin EXCEPT gsd-commands so all 13 capabilities are
    ...
    // register the command) while the other 14 commands register normally.
    ...
    assert.ok(ctx2.commands.length === 14, `expected 14 commands, got ${ctx2.commands.length}`);
...
describe("mount: cordis.patch.yml rows resolve", () => {
  test("override row present, 15 insert rows resolve via exports + import()", async () => {
    ...
    // Exactly the 15 insert rows (D-03).
    assert.ok(insertRows.length === 15, `expected 15 insert rows, got ${insertRows.length}`);
    assert.deepEqual(insertRows, EXPECTED_INSERT_ROWS, "parsed insert rows differ from the expected 15");
...
describe("mount: reactive loop rendering (DEGR-02/DEGR-04)", () => {
  ...
  test("full-set mount still renders present steps + tools (regression, D-11)", async () => {
    const { ctx } = await mountSubset(["persona", "state", "core-tools", "discuss", "spec", "plan", "gap-analysis", "execute", "code-review", "verify", "ship", "ui", "quick", "map-codebase"], { subagents: makeSubagents() });
    ...
    const snap = snapshot(ctx);
    assert.match(snap, /Available steps: spec, discuss, ui, plan, gap-analysis, quick, execute, code-review, verify, ship\./);
