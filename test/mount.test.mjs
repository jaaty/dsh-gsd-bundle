...
//
// Proves the 27 cordis.patch.yml plugin rows activate inside a fake DSH host:
...
// context provider, gsdState service, 37 gsd_* tools, 34 /gsd-* commands).
...
// Apply all 27 plugins in patch order against a ctx; throw with the offending id.
...
// The 27 expected insert rows, verbatim from cordis.patch.yml (D-03:
...
// Expected registered tool names (37) — verified against the real modules.
const EXPECTED_TOOL_NAMES = [
  ...
  "gsd_quick", "gsd_quick_batch", "gsd_fast_mode", "gsd_mvp_phase", "gsd_map_codebase", "gsd_job", "gsd_intel_updater",
...
// Expected registered command names (34) — from lib/commands.js (D-03).
const EXPECTED_COMMAND_NAMES = [
  ...
  "gsd-quick", "gsd-quick-batch", "gsd-fast-mode", "gsd-mvp-phase", "gsd-map-codebase", "gsd-new-milestone",
...
describe("mount: all 27 plugins activate", () => {
  let fs, ctx;
  beforeEach(() => {
    fs = new FakeFs();
    // Supply subagents so the gsd_job sub-fiber activates (DEGR-07 D-05) and
    // the full 25-row surface is registered.
    ctx = makeMountCtx(fs, { subagents: makeSubagents() });
  });

  test("applies all 27 plugins in patch order without throwing", async () => {
    await applyAll(ctx);
    ...
    assert.ok(ctx.tools.length === 37, `expected 37 tools, got ${ctx.tools.length}`);
    assert.ok(ctx.commands.length === 34, `expected 34 commands, got ${ctx.commands.length}`);
    ...
    // DEGR-01: all 28 capability services are provided with the documented
    ...
    assert.ok(CAPABILITY_KEYS.length === 28, `expected 28 capability keys, got ${CAPABILITY_KEYS.length}`);
  ...
  test("absent capability leaves its slash command unregistered (DEGR-03)", async () => {
    // Apply every plugin EXCEPT gsd-commands so all 28 capabilities are
    ...
    // register the command) while the other 33 commands register normally.
    ...
    assert.ok(ctx2.commands.length === 33, `expected 33 commands, got ${ctx2.commands.length}`);
...
describe("mount: cordis.patch.yml rows resolve", () => {
  test("override row present, 27 insert rows resolve via exports + import()", async () => {
    ...
    // Exactly the 27 insert rows (D-03).
    assert.ok(insertRows.length === 27, `expected 27 insert rows, got ${insertRows.length}`);
    assert.deepEqual(insertRows, EXPECTED_INSERT_ROWS, "parsed insert rows differ from the expected 27");
    ...
    // Cross-check captured tool names against the expected 37.
    const fs = new FakeFs();
    ...
    // Cross-check captured command names against the expected 34.
...
describe("mount: persona orients at STATE.md (MOUNT-02)", () => {
  ...
  test("all 37 registered tools have a valid compiled schema", () => {
    // apply() not throwing already proves defineTool compiled the schema (D-04);
    // assert the shape explicitly for every tool.
    assert.ok(ctx.tools.length === 37, "expected 37 registered tools");
...
describe("mount: reactive loop rendering (DEGR-02/DEGR-04)", () => {
  ...
  test("full-set mount still renders present steps + tools (regression, D-11)", async () => {
    const { ctx } = await mountSubset(["persona", "state", "core-tools", "discuss", "spec", "plan", "gap-analysis", "execute", "code-review", "ui-review", "verify", "validate", "undo", "ship", "milestone-audit", "learnings", "graphify", "mempalace", "autonomous", "add-tests", "repair", "ui", "quick", "map-codebase", "health", "phase-management"], { subagents: makeSubagents() });
    ...
    // validate order 45 slots between verify and ship).
    const snap = snapshot(ctx);
    assert.match(snap, /Available steps: spec, discuss, ui, plan, gap-analysis, quick, execute, code-review, ui-review, verify, validate, ship, milestone-audit, learnings, graphify, mempalace\./);
