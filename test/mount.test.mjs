...
//
// Proves the 20 cordis.patch.yml plugin rows activate inside a fake DSH host:
...
// context provider, gsdState service, 22 gsd_* tools, 19 /gsd-* commands).
...
// Apply all 20 plugins in patch order against a ctx; throw with the offending id.
...
// Expected registered tool names (22) — verified against the real modules.
const EXPECTED_TOOL_NAMES = [
  ...
  "gsd_health", "gsd_milestone_audit",
];
...
describe("mount: all 20 plugins activate", () => {
  let fs, ctx;
  beforeEach(() => {
    fs = new FakeFs();
  ...
  test("applies all 20 plugins in patch order without throwing", async () => {
    await applyAll(ctx);
    ...
    assert.ok(ctx.tools.length === 22, `expected 22 tools, got ${ctx.tools.length}`);
    ...
    assert.ok(CAPABILITY_KEYS.length === 18, `expected 18 capability keys, got ${CAPABILITY_KEYS.length}`);
...
describe("mount: cordis.patch.yml rows resolve", () => {
  test("override row present, 20 insert rows resolve via exports + import()", async () => {
    ...
    assert.ok(insertRows.length === 20, `expected 20 insert rows, got ${insertRows.length}`);
    assert.deepEqual(insertRows, EXPECTED_INSERT_ROWS, "parsed insert rows differ from the expected 20");
...
describe("mount: reactive loop rendering (DEGR-02/DEGR-04)", () => {
  ...
  test("full-set mount still renders present steps + tools (regression, D-11)", async () => {
    const { ctx } = await mountSubset(["persona", "state", "core-tools", "discuss", "spec", "plan", "gap-analysis", "execute", "code-review", "ui-review", "verify", "validate", "undo", "ship", "milestone-audit", "ui", "quick", "map-codebase", "health"], { subagents: makeSubagents() });
    ...
    const snap = snapshot(ctx);
    assert.match(snap, /Available steps: spec, discuss, ui, plan, gap-analysis, quick, execute, code-review, ui-review, verify, validate, ship, milestone-audit\./);
